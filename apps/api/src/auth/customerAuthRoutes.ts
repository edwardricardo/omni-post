...
import { resolveClientIp } from "../security/httpRateLimitPreHandler.js";
...
class CustomerAuthRouteHandler extends BaseRouteHandler {
  ...
  /**
   * @method resolveIp
   * @description Derive the trusted client IP from the configured proxy-hop
   *   count — NEVER `request.ip` (with `trustProxy`, that is the spoofable
   *   leftmost `X-Forwarded-For`). Used for MFA challenge IP-binding and the
   *   brute-force forensic IP on both login steps.
   */
