...
import type { CustomerRequestUser } from "./customerAuthMiddleware.js";
 ...
 * @description Reads the bearer credential out of the Authorization header.
 * @param request - The incoming Fastify request.
 * @returns The token string, or null when the header is absent, not a Bearer scheme, or empty.
 */
 ...
 * @param request - The incoming Fastify request.
 * @param reply - The Fastify reply, used only on the rejection paths.
 * @returns Nothing; returning without sending lets the route handler run.
 */
export async function requireCustomerOrAdminAuth(
  ...
  try {
    ...
    const customerUser: CustomerRequestUser = {
      id: payload.sub,
    ...
    // Bind tenant context so downstream tenant-guarded queries run scoped to
    // this account — same as requireClientAuth.
  ...
  if (adminAuthService) {
    ...
    if (result.ok) {
      ...
      request.auth = {
        ...
        sessionId: "", // Not available from an access token, mirroring requireAdminAuth.
