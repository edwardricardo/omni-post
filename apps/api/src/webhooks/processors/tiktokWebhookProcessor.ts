...
export class TikTokWebhookProcessor extends AbstractWebhookProcessor {
  ...
  override async parse(payload: Record<string, any>): Promise<{
    ...
    const eventTypeStr = event.type || event.event_type;

    switch (eventTypeStr) {
      case "video.create":
      case "video.publish":
        eventType = "POST_PUBLISHED";
        normalizedData = await this.parseVideoCreateEvent(event.content);
        break;

      case "video.remove":
      case "video.delete":
        eventType = "POST_DELETED";
        normalizedData = await this.parseVideoRemoveEvent(event.content);
        break;

      case "comment.create":
        eventType = "COMMENT_RECEIVED";
        normalizedData = await this.parseCommentEvent(event.content);
        break;

      case "comment.reply":
        eventType = "COMMENT_RECEIVED";
        normalizedData = await this.parseCommentReplyEvent(event.content);
        break;

      case "user.authorization.revoke":
        eventType = "ACCOUNT_DISCONNECTED";
        normalizedData = await this.parseAuthRevokeEvent(event.content);
        break;

      case "video.statistics.update":
        eventType = "POST_ENGAGEMENT_UPDATE";
        normalizedData = await this.parseVideoStatsEvent(event.content);
        break;

      default:
        eventType = "POST_UPDATED";
        normalizedData = { eventType: eventTypeStr, content: event.content };
    }

    // Find related entities based on TikTok user ID or video ID
    relatedEntities = await this.findRelatedEntities(event.content, normalizedData);
   ...
   */
  private async parseVideoCreateEvent(content: any): Promise<Record<string, any>> {
    return {
   ...
   */
  private async parseVideoRemoveEvent(content: any): Promise<Record<string, any>> {
    return {
   ...
   */
  private async parseCommentEvent(content: any): Promise<Record<string, any>> {
    return {
      eventType: "comment_received",
   ...
   */
  private async parseCommentReplyEvent(content: any): Promise<Record<string, any>> {
    return {
   ...
   */
  private async parseAuthRevokeEvent(content: any): Promise<Record<string, any>> {
    return {
   ...
   */
  private async parseVideoStatsEvent(content: any): Promise<Record<string, any>> {
    return {
   ...
   */
  private async handleVideoCreated(data: Record<string, any>, entities: any): Promise<void> {
    ...
    if (entities.accountId && entities.projectId && channelId) {
      await prisma.analytics.create({
        data: {
          ...
          comments: 0,
          shares: 0,
          capturedAt: new Date(data.createdAt || data.publishedAt),
        },
      });
    }
  }
   ...
   */
  private async handleVideoRemoved(data: Record<string, any>, entities: any): Promise<void> {
    const { postId, channelId } = entities;
    ...
    if (entities.accountId && entities.projectId && entities.channelId && data.videoId) {
      ...
      const existing = await prisma.analytics.findFirst({
        where: {
          ...
          provider: "TIKTOK",
          ...(entities.postId && { postId: entities.postId }),
        },
      });
   ...
   */
  private async handleAuthRevoked(data: Record<string, any>, entities: any): Promise<void> {
    const { channelId } = entities;
   ...
   */
  private async handleVideoStatsUpdate(data: Record<string, any>, entities: any): Promise<void> {
    if (entities.accountId && entities.projectId && entities.channelId && data.videoId) {
      ...
      if (existing) {
        ...
        if (this.broadcaster && entities.postId) {
          await this.broadcaster.broadcastEngagementUpdate(
            ...
            {
              views: data.views - oldMetrics.views,
              likes: data.likes - oldMetrics.likes,
              comments: data.comments - oldMetrics.comments,
              shares: data.shares - oldMetrics.shares,
            }
          );
        }
      } else {
