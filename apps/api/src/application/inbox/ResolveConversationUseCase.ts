...
export class ResolveConversationUseCase
  implements UseCase<ResolveConversationInput, void, UseCaseError>
{
  ...
  async execute(input: ResolveConversationInput): Promise<Result<void, UseCaseError>> {
    ...
    if (!resolveResult.ok) {
      return err(
        new UseCaseError(resolveResult.error.message, USE_CASE_ERRORS.CONFLICT, resolveResult.error)
      );
    }
