...
export class GenerateImageUseCase {
  ...
  async execute(input: GenerateImageInput): Promise<Result<GeneratedImageData, UseCaseError>> {
    ...
    // Delegate to AI service for image generation
    ...
    if (!aiResult.ok) {
      const errorMsg =
        typeof aiResult.error === "object" && aiResult.error !== null && "message" in aiResult.error
          ? (aiResult.error as { message: string }).message
          : "Image generation failed";
      return err(new UseCaseError(errorMsg, USE_CASE_ERRORS.INTERNAL_ERROR));
    }

    const generatedValue = aiResult.value;
    if (!generatedValue) {
      return err(
        new UseCaseError("AI service returned no image data", USE_CASE_ERRORS.INTERNAL_ERROR)
      );
    }
