/**
 * @file PrismaCustomerMfaUserRepository.ts
 * @description Prisma adapter implementing MfaUserRepositoryPort against the CustomerUser
 ...
 *              subject discriminator lives in the service, not here. `customerUser` is a
 *              tenant-scoped model (TENANT_SCOPED_MODELS): when an authenticated customer
 *              request is running (TenantContext bound by `requireClientAuth`), the Prisma
 *              tenant guard extension auto-scopes every query here to that account.
...
/**
 * @class PrismaCustomerMfaUserRepository
 * @description CustomerUser-backed implementation of the MFA persistence port.
 */
export class PrismaCustomerMfaUserRepository implements MfaUserRepositoryPort {
  ...
  async findById(userId: string): Promise<Result<MfaUserRecord, "NOT_FOUND">> {
    const row = await this.getClient().customerUser.findUnique({
      where: { id: userId },
      select: {
        id: true,
        email: true,
  ...
  async markBackupCodeUsed(
    ...
    const row = await client.customerUser.findUnique({
      where: { id: userId },
  ...
  private async update(
    ...
    try {
      await this.getClient().customerUser.update({ where: { id: userId }, data });
      return ok(undefined);
    } catch (error: unknown) {
      if (
        typeof error === "object" &&
