...
describe("processTriageInboxJob", () => {
  ...
  it("binds the payload's account as the tenant context for the whole run", async () => {
    ...
    // triage adapters read `socialMessage` and `crmContact` — both tenant-guard-enrolled — on the
    // container's guarded client, so dropping the binding makes every one of those reads throw.
