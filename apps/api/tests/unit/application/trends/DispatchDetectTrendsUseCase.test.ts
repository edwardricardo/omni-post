/**
 * @file dispatchDetectTrends.test.ts
 * @description Unit tests for the trend-radar coordinator: one job per
 *              distinct account with active channels, date-bucketed dedupe
 *              key, enqueue-failure accounting, and the UnitOfWork-wrapped
 *              path.
...
import { DispatchDetectTrendsUseCase } from "../../../../src/application/trends/DispatchDetectTrendsUseCase.js";
...
describe("DispatchDetectTrendsUseCase", () => {
  ...
  it("enqueues one job per distinct account with active channels", async () => {
    ...
    const uc = new DispatchDetectTrendsUseCase(channelQuery, queue, "trend-radar");
    ...
    const dedupeKeys = (calls as Array<{ dedupeKey: string; payload: { accountId: string } }>)
      .map((c) => c.dedupeKey)
      .sort();
    assert.deepStrictEqual(dedupeKeys, [`trend-radar-acc-1-${day}`, `trend-radar-acc-2-${day}`]);
  });
  ...
  it("scopes dispatch to a single account when accountId is given", async () => {
