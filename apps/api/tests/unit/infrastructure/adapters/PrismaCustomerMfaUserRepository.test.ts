...
describe("PrismaCustomerMfaUserRepository", () => {
  ...
  describe("markBackupCodeUsed", () => {
    ...
    it("refuses a replayed index and leaves the first consumption's timestamp in place", async () => {
      ...
      const result = await repo.markBackupCodeUsed("customer-1", 0, replayAt);

      expect(result.ok).toBe(false);
      expect(!result.ok && result.error).toBe("ALREADY_USED");
      expect(rows.get("customer-1")?.mfaBackupUsedAt).toEqual({ "0": firstClaimedAt });
    });
    ...
    it("returns ALREADY_USED when a concurrent writer changed the used-map (CAS count 0)", async () => {
      ...
      const honest = (
        fake.prisma as unknown as {
          customerUser: {
            findUnique: (args: { where: { id: string } }) => Promise<FakeCustomerUserRow | null>;
            updateMany: (args: {
        ...
        customerUser: {
          findUnique: async (args: {
            ...
            if (!writerCommitted) {
              ...
              const stored = fake.rows.get("customer-1");
              if (stored) {
                fake.rows.set("customer-1", { ...stored, mfaBackupUsedAt: concurrentMap });
              }
            }
            return snapshot;
          },
      ...
      const raceRepo = new PrismaCustomerMfaUserRepository(racedPrisma);
      ...
      expect(fake.rows.get("customer-1")?.mfaBackupUsedAt).toEqual(concurrentMap);
    });
