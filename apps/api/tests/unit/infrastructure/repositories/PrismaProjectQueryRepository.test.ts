...
describe("getProjectAccess behavioral gate closure (honest count semantics)", () => {
  ...
  /**
   * Honest fake: applies the equality semantics Postgres applies to the exact
   * fields this gate queries (id, accountId, deletedAt). Unlike the shared
   * permissive mock, an unfiltered where here MATCHES the soft-deleted row —
   * so this test only passes while the production where excludes it.
   */
  function makeCountingPrisma(rows: ProjectRow[]) {
    return {
      project: {
        count: vi.fn(
          async ({ where }: { where: Partial<Record<keyof ProjectRow, unknown>> }) =>
            rows.filter((row) =>
              (Object.keys(where) as (keyof ProjectRow)[]).every((key) => row[key] === where[key])
            ).length
        ),
      },
    };
  ...
  it("gate is open while the project is live, and CLOSES once it is soft-deleted", async () => {
    const project: ProjectRow = { id: "proj-1", accountId: "acc-1", deletedAt: null };
    const prisma = makeCountingPrisma([project]);
    const repo = new PrismaProjectQueryRepository(prisma as never);

    expect(await repo.getProjectAccess("acc-1", "proj-1")).toBe(true);

    // Soft-delete the project — the same row now carries deletedAt.
    project.deletedAt = new Date("2026-09-05T00:00:00Z");

    expect(await repo.getProjectAccess("acc-1", "proj-1")).toBe(false);
  });
});
