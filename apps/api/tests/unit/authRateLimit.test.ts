 ...
 *
 * Verifies that POST /auth/login, /auth/refresh, and /auth/logout correctly
 * enforce per-route rate limits configured via config.rateLimit in authRoutes.ts.
 *
 * Design:
...
describe("Auth endpoint rate limiting", () => {
  ...
  describe("Rate-limit isolation between routes", () => {
    ...
    it("exhausting /auth/login limit should NOT affect /auth/logout limit", async () => {
      ...
      // Logout from the same IP must still be allowed (separate per-route counter)
      const logoutAllowed = await postLogout(app, sharedIp);
      expect(logoutAllowed.statusCode).not.toBe(429);
    });
  });
});
