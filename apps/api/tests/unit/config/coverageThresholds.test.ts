 ...
 *              the 55/45 global floor so coverage is gated once on merged data.
...
describe("buildCoverageThresholds", () => {
  describe("default (non-sharded) run", () => {
    it("returns the 55/45 global floor when sharding is disabled", () => {
      ...
      expect(thresholds?.statements).toBe(55);
    });
