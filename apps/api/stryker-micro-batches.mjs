...
function generateConfig(batch) {
  const excludes = batch.mutate
    ...
    .flatMap((p) => [p.replace("**/*.ts", "**/*.test.ts"), p.replace("**/*.ts", "**/*.spec.ts")]);
...
async function runBatch(batch) {
  ...
  try {
    const output = execSync(`pnpm exec stryker run ${configPath} 2>&1`, {
      timeout: 7200_000,
    ...
    const summaryMatch = output.match(
      /All files\s*\|\s*[\d.]+\s*\|\s*[\d.]+\s*\|\s*(\d+)\s*\|\s*(\d+)\s*\|\s*(\d+)\s*\|\s*(\d+)/
    );
    ...
    log(
      `✅ ${batch.id} DONE: score=${score}% | killed=${killed} timeout=${timeout} survived=${survived} noCov=${noCov} | ${duration}`
    );
...
const batches = targetBatch ? MICRO_BATCHES.filter((b) => b.id === targetBatch) : MICRO_BATCHES;
