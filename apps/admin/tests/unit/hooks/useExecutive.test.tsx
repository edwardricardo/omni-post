/**
 * Tests for useExecutive
 *
 * The hook calls fetch directly to /api/backend/api/admin/executive/metrics
...
import type { ExecutiveSummary } from "@/hooks/api/useExecutive";
...
describe("useExecutive", () => {
  ...
  it("fetches and maps executive metrics for default 30d range", async () => {
    ...
    const data = result.current.data as ExecutiveSummary;
  ...
  it("calls the correct URL with startDate/endDate params", async () => {
    ...
    expect(calledUrl).toContain("/api/backend/api/admin/executive/metrics");
  ...
  it("uses the timeRange in the query key", async () => {
    ...
    const { result } = renderHook(() => useExecutive("90d"), { wrapper });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    const cached = queryClient.getQueryData(["executive", "summary", "90d"]);
  ...
  it("throws when body.ok is false", async () => {
    ...
    await waitFor(() => expect(result.current.isError).toBe(true));
    expect((result.current.error as Error).message).toBe("Failed to fetch executive metrics");
  });
