-- Enroll RecurringPost in the tenant guard (tenant-guard rollout, Slice 3).
...
-- companion RLS migration (20260714040300_add_rls_recurring_post), whose policy references the
-- "accountId" column added here.

-- 1. Add nullable so existing rows survive the ADD.
ALTER TABLE "RecurringPost" ADD COLUMN "accountId" TEXT;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM "RecurringPost" WHERE "accountId" IS NULL) THEN
    RAISE EXCEPTION 'Backfill incomplete: RecurringPost has rows with NULL accountId after the Project join';
  END IF;
END $$;

-- 4. Tighten to NOT NULL (atomic with the backfill inside Prisma's per-migration transaction).
ALTER TABLE "RecurringPost" ALTER COLUMN "accountId" SET NOT NULL;

-- 5. Foreign key + index (Prisma naming convention).
ALTER TABLE "RecurringPost"
  ADD CONSTRAINT "RecurringPost_accountId_fkey"
  FOREIGN KEY ("accountId") REFERENCES "Account"("id") ON DELETE CASCADE ON UPDATE CASCADE;

CREATE INDEX "RecurringPost_accountId_projectId_idx" ON "RecurringPost"("accountId", "projectId");
