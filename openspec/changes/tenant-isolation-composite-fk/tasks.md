...
- [ ] 6b.1 [RED · ALREADY ON RECORD — re-run, never re-manufacture] Re-run the ADR-0022 §Runtime cutover two-channel measurement (same 18-suite batch, same concurrency, only the role differing) with `DATABASE_URL` pointed at `omnipost_app`: expect `177 tests · pass 170 · fail 7 · cancelled 0`, the 7 confined to `postDeleteOwnership.test.ts` (3) + `postReadOwnership.test.ts` (4). Record the re-run in the PR body — the spec requirement forbids manufacturing a fresh red for this.
- [ ] 6b.2 [RED · unit] Pin 6a.1's spike outcome in-repo: a vitest suite over the chained extension asserting the working order (guard-then-bind), the two-statement batch shape, and PASS-THROUGH when `isGucBound()` is true. Red until 6b.3.
- [ ] 6b.3 [GREEN][SENSITIVE] Create `infra/prisma/src/extensions/tenantGucBinding.ts` — `tenantGucBindingExtension(provider)`. Per model operation: scope = `getSystemContext()` → `__system__`, else `getTenantContext()` → `accountId`. Scope present AND `isGucBound()` false → `client.$transaction([$executeRaw set_config('app.account_id', scope, true), query(args)])`. Marker set → pass through (the operation stays on the ambient transaction's connection). NO scope → pass through UNWRAPPED, loud by construction (the guard throws on any enrolled model). **ALL models bind, deliberately**: skipping unenrolled roots would leave `findOwnerAccountId` (root `post`, JOIN into RLS-covered `Project`) unbound — the exact measured failure.
- [ ] 6b.4 [GREEN] `apps/api/src/infrastructure/container/setup.ts`: chain the binding extension with the existing `tenantGuardExtension` on the same provider; the extended client stays registered as `TOKENS.PrismaClient`. Run VITEST 6b.2 → green.
- [ ] 6b.5 [RED → GREEN] `apps/api/src/infrastructure/repositories/PrismaPostRepository.ts:437` (`findOwnerAccountId`): write the failing case FIRST (a select whose `project` relation returns null is dereferenced today), then return a null-checked NotFound-shaped result. ADR-0022 deferred this to whoever took the binding decision precisely so it could not MASK the cutover red; with binding landed, the masking objection is gone.
- [ ] 6b.6 [GREEN] Re-run 6b.1's batch on the app-role channel → **177/177, exit 0**, the 7 green. Record it in ADR-0022 §Runtime cutover as the INTERIM measurement; the blocked table flips to "landed" only at 6d.8, with the flip itself.
- [ ] 6b.7 [ORCHESTRATOR · SENSITIVE token] Fitness **#40 — one transaction seam (ALLOWLIST, the #28 form)**: add it to `CLAUDE.md` §Automated Compliance Checks and MIRROR it verbatim into `.github/workflows/fitness.yml` (paste, never paraphrase — drift between doc and workflow is the failure mode). Every `.$transaction(` in `apps/api/src` + `packages/adapters/db-prisma/src` MUST be a `withGucBoundTransaction` call; the only exclusions are the helper's own file (`infra/prisma/src/extensions/tenantGuc.ts`), the named seams (`PrismaUnitOfWork.ts`, `sagaTenant.ts`, `db-prisma/ChannelRepository.ts`), workers, and tests. **Allowlist, not denylist, for the #28 reason**: the dangerous set is open-ended (a raw batch, a hand-rolled interactive tx, any future helper-skipping call), so the check names the ONE admissible form instead of predicting the rest. **Home is fitness, not the integration tier**: unlike the pg-catalog gate (D-S0-4 — `relrowsecurity` is database state a grep cannot read), this invariant is a CALL SHAPE in source, which is exactly what a grep decides.
- [ ] 6b.8 [RED-PROOF · mandatory, canon step 3] Plant a bare `.$transaction(` at one converted site → the gate exits NON-ZERO for real (an `::error` annotation alone leaves the job green and proves nothing) → restore the tree byte-exact (verify with `cmp`) → re-confirm count 0. Then confirm count = 0 at the branch tip over the whole swept scope (canon step 4). Record both in the PR body.
- [ ] 6b.5b [RED → GREEN · from the 0d-1a gate review, WARNING-1] **The UoW atomicity shape** — the spec's SECOND shape (rls-enforcement "exercised on BOTH shapes"), deferred by 0d-1a to a named owner: THIS task. Extend `tenantGucTransactionBinding.test.ts` with a write + forced failure through `PrismaUnitOfWork.executeInTransaction` WITH the binding extension composed, asserting rollback-together AND the marker held (`markerHeldInsideUoW`). RED = removing the `runWithBoundGuc` adoption from `PrismaUnitOfWork.ts:94` (the escape it prevents goes live exactly when 6b.3 lands, which is why the task lives here). Uses the probe-extension technique the suite already carries.
- [ ] 6b.5c [ADJUDICATE per site · from the gate review] Nesting under the binding: once 6b.3 lands, a helper call nested inside a UoW callback changes character under the app role (from fails-closed-unbound to succeeds-bound-but-non-atomic). The ternary arms cannot nest (UoW-aware via `activeTx`, verified); `EventStore.append` CAN. Adjudicate each nestable site: document the intended semantics beside the call or restructure. Fitness #40 gates the seam, not the nesting — this task is the nesting's owner.
- [ ] 6b.9 **0-defect gate (PR 0d-1b)**: TSC = 0; `eslint --max-warnings 0` = 0; fitness #8/#9/#10/#21/#23/#30/#32/#38/#39/**#40** = 0; the `integration:tenant-isolation` batch 177/177 on BOTH channels; VITEST touched unit set green, 0 cancelled/0 skipped; prettier clean.
- [ ] 6b.10 [FOLLOW-UP note, from the gate review] `getAmbientGucScope()` is a convention, not a chokepoint: nothing stops a future site from calling `getTenantContext()` directly and minting its own scope. When #40 is authored (6b.7), assess widening it (or a sibling check) to gate scope DERIVATION as well; if deferred, record the residual in the fitness comment rather than silently.
