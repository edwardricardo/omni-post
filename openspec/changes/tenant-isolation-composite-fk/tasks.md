...
- [ ] 6a.1 [SPIKE · hour-boxed · scratchpad only, nothing shipped] Prove batch enlistment of `query(args)` through a CHAINED `$extends` (guard + binding) and pin the working order; record outcome + decision in the PR body. Misbehaviour → the design's NAMED fallback (fold the binding into `tenantGuardCheck`'s wrapper — one extension, same seam), which re-shapes 6b.3 and nothing else. The rejected alternative is not re-litigated: binding on the ambient itx client executes as a SAVEPOINT, and `set_config(..., true)` REVERTS when the subtransaction ends.
- [ ] 6a.2 [OWN COMMIT · re-measure, never trust the snapshot] Re-run the `\.\$transaction\(` enumeration over `apps/api/src` + `packages/adapters/db-prisma/src`, minus the named seams (`PrismaUnitOfWork`, `sagaTenant.ts`, `db-prisma/ChannelRepository.ts`), workers, and tests. Pin the delta against the design's 19-site snapshot **as re-gated: 12 interactive / 7 batch (NOT 14/5)** and record it in the PR body. A site the re-run surfaces is adopted in THIS link, never deferred.
- [ ] 6a.3 [RED · the ESCAPE class, named on a NON-UoW site] Create `apps/api/tests/integration/tenantGucTransactionBinding.test.ts` (node:test, real DB, `@file`/`@description`/`@layer infrastructure` header — fitness #9/#10): drive `PrismaPostRepository.create` down its UoW-aware FALLBACK arm (`:453`) and force the nested `postContent.create` to fail on the extended client. A `Post` row SURVIVING the failure proves an inner operation escaped onto a second connection and committed. RED while `:453` opens a bare `$transaction`. This is the `rls-enforcement` atomicity scenario's repository-opened shape; the unit-of-work shape stays pinned separately.
- [ ] 6a.4 [GREEN][SENSITIVE] `infra/prisma/src/extensions/tenantGuc.ts`: add the `AsyncLocalStorage` marker `runWithBoundGuc(scope, fn)` + `isGucBound()` — semantics "the ambient transaction OWNS GUC adjudication; do not wrap" — and `withGucBoundTransaction(client, scope, fn)`, which opens the interactive transaction, binds via `set_config('app.account_id', scope, true)` WHEN a scope exists, and sets the marker in **ALL** branches including the deliberate-unbound one (a transaction that binds nothing still owns its connection, and its inner operations would still escape).
- [ ] 6a.5 [GREEN] Adopt the marker at the named seams: `apps/api/src/infrastructure/unitofwork/PrismaUnitOfWork.ts` (`executeInTransaction`, wrapping `txStorage.run`) and `apps/api/src/saga/sagaTenant.ts` (`runSagaTenantTransaction` + `runSagaSystemTransaction`). The same touch fixes the stale `PrismaUnitOfWork.ts:71` comment — "the 51 tenant-scoped tables" against a 58-model guard Set — to the count-free wording the archived `api-guarded-client-injection` change adjudicated but never landed.
- [ ] 6a.6 [GREEN] Route the **12 interactive** sites through `withGucBoundTransaction`: `PrismaPostRepository.ts:453` / `:507` / `:156`, `PrismaProjectRepository.ts:376`, `PrismaAccountRepository.ts:402` (those last three plus `:156` are the UoW-aware ternary FALLBACK arms), `EventStore.ts:79`, `SchedulingPostHandlers.ts:246` + `:342`, `PrismaMediaAssetRepository.ts:216`, `PrismaApprovalWorkflowRepository.ts:121`, `PrismaApprovalRequestRepository.ts:153`, `packages/adapters/db-prisma/src/PostRepository.ts:94` (raw client today — no escape hazard, but the helper gives it binding + marker in one move). Line numbers are 6a.2's re-measured set, not the snapshot's.
- [ ] 6a.7 [GREEN] Convert the **7 batch** sites to the helper's interactive form — same atomicity, none of the batch-enlistment ambiguity: `OutboxClaimService.ts:141`, `outboxAdminRoutes.ts:60` (its `prisma` resolves `TOKENS.PrismaClient` — extended, not raw), `gatewaySwitchProcessor.ts:113`, `PrismaTrackedLinkRepository.ts:159` + `:176`, plus the two the re-gate moved out of the interactive column, each needing an **array → sequential-await restructure**: `SchedulingSlotHandlers.ts:412` and `PrismaAnalyticsWriteRepository.ts:57`.
- [ ] 6a.8 [GREEN] Seam wiring: `apps/api/src/infrastructure/container/setup.ts` curries the helper with the SAME `TenantContextProvider` the guard reads (one provider, no second source of tenant truth); workers and `db-prisma` pass scope EXPLICITLY, their existing convention.
- [ ] 6a.9 [GREEN] Run INT 6a.3 → green (write and forced failure roll back TOGETHER). Add VITEST marker unit tests: the marker is set in all three branches (tenant / `__system__` / deliberate-unbound), `isGucBound()` is false outside any adopted transaction, and a nested helper call binds at most once.
- [ ] 6a.10 `apps/api/scripts/run-tests.sh`: add `tenantGucTransactionBinding.test.ts` to the `integration:tenant-isolation` `run_batch` (fitness #30 — a suite no batch names never executes).
- [ ] 6a.11 **0-defect gate (PR 0d-1a)**: TSC = 0; `eslint --max-warnings 0` = 0; fitness #8/#9/#10/#21/#23/#30/#32/#38/#39 = 0; INT 6a.3 green plus the existing `integration:tenant-isolation` batch UNCHANGED on the owner channel (this link claims no behaviour change), 0 cancelled/0 skipped; prettier clean.
