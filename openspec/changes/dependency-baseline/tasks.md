...
- [ ] **T3.5** `@opentelemetry/api-logs` → single via **`pnpm.overrides`** (5 versions in store; transitive, NOT declared → override, NOT catalog — design §2.4). Add explicit `"@opentelemetry/api-logs": "<resolved>"`. **DoD:** spec scenario "each previously drifting name resolves once" — `@opentelemetry/api-logs` collapses from 5 to 1 version; entry under `pnpm.overrides`. File: root `package.json`. _OOM-safe edit._
...
- [ ] **T4.1 [PAUSE-STACK]** `04a-toolchain` — bump `typescript`, `vitest`, `eslint*`, `prettier`, `@types/*` via `taze -l --maturity-period 7`, atomically. **DoD:** spec "no pre-release identifiers"; compile + lint + 27 fitness green; `--frozen-lockfile` before `04b`. PR `dep-baseline/04a-toolchain`. _Build/test._
- [ ] **T4.2 [PAUSE-STACK]** `04b-otel` — bump the whole `catalogs.otel` set together. **DoD:** spec "each locked family resolves to a single compatible set"; metrics endpoint + span emission verified in integration tier; family atomic (syncpack single-version catches a half-bump). PR `dep-baseline/04b-otel`. _Build/test._
- [ ] **T4.3 [PAUSE-STACK]** `04c-react` — bump `react` + `react-dom` + `@types/react` + `@types/react-dom`, atomic. **DoD:** spec "react and react-dom share one version"; frontend unit tests + size-limit pass. PR `dep-baseline/04c-react`. _Build/test._
- [ ] **T4.4 [PAUSE-STACK]** `04d-tiptap` — bump `@tiptap/*` + `prosemirror-*`, atomic. **DoD:** editor stories render; family single-version. PR `dep-baseline/04d-tiptap`. _Build/test._
- [ ] **T4.5 [PAUSE-STACK]** `04e-langchain` — bump `@langchain/*`, atomic. **DoD:** AI eval tier passes; family single-version. PR `dep-baseline/04e-langchain`. _Build/test._
- [ ] **T4.6 [PAUSE-STACK]** `04f-googleapis` — bump `googleapis` + `google-auth-library` (+ the `@10` / `gaxios@7` scoped overrides bumped in lockstep). **DoD:** youtube provider contract tests pass; the scoped overrides still target their distinct major. PR `dep-baseline/04f-googleapis`. _Build/test._
- [ ] **T4.7 [PAUSE-STACK]** `04g-runtime` — bump `@prisma/client`, `opossum`, `prom-client`, `fastify`, `pino`, `bullmq`, `ioredis`. Runtime-critical — full integration + saga + RLS tiers. Lands LAST. **DoD:** spec "a family bump is applied atomically with verify-after-each"; all runtime tiers green; fitness #25 (opossum) holds. PR `dep-baseline/04g-runtime`. _Full integration._
- [ ] **T4.8 [PAUSE-STACK]** `04h-misc` — remaining single-package bumps (`zod`, `next`/`next-intl`, `cloudinary`, `handlebars`, `uuid`, `validator`, etc.). **DoD:** spec "catalog pins match the latest stable dist-tag [runtime]"; build/test/27 fitness green. PR `dep-baseline/04h-misc`. _Build/test._
