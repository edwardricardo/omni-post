...
**Status — all committed + CI-green on `workstream/dep-baseline`:**
...
- **REMAINING:** **Step 5** (below) + tracker→main PR + the re-validation WALK.
...
- [x] **T5.1** Author `.syncpackrc.json` at repo root (design §5.1): workspace-protocol ignore group + single-version `highestSemver` group + `semverGroups.range: ""` exact-range gate. **DoD:** `syncpack lint` keys both invariants. File: `.syncpackrc.json` (new). _OOM-safe._ _DONE — config was already authored in T1.14; this slice added documenting `label`s recording the scoping decision. RECONCILIATION (ADR-0018): the single-version group over `["**"]` only sees MANIFEST-DECLARED specs (all `catalog:`/`catalog:otel` refs + root literals), so consumer-governed TRANSITIVES (vite 7.3.5+8.x, minimatch multi-versions) are STRUCTURALLY INVISIBLE to it and are not flagged — exactly the desired behavior. `syncpack@12 list-mismatches` exits **0** (605 valid, 427 workspace ignored, 0 mismatches). NOTE: `syncpack lint` (≠ list-mismatches) reports every `catalog:` ref as `UnsupportedMismatch` (509 of them, ALL catalog-protocol, ZERO real range violations) — a syncpack@12 limitation, so the CI gate (T5.2) uses `list-mismatches`. Exactness for catalog deps is guaranteed by `catalogMode: strict` + `save-prefix=""` + exact catalog values._
- [BLOCKED] **T5.2** Add the `dependency-consistency` job to `.github/workflows/fitness.yml` (design §5.2 / D5 — fitness.yml is the invariant home, NOT a new workflow): steps for `syncpack list-mismatches`, `pnpm install --frozen-lockfile`, `pnpm dedupe --check`. **DoD:** spec scenario "the three CI gate steps are wired [static]" — three gating steps present, each fails the PR on non-zero exit. File: `.github/workflows/fitness.yml`. _OOM-safe._ _GUARD-BLOCKED — `.github/workflows/` is a sensitive path (`pre_edit.py` SENSITIVE_PATTERNS); requires Edward's `omnipost-allow sensitive-edit` token. Edit prepared but NOT applied: a `dependency-consistency` job (checkout + setup-node-pnpm-cache composite + 3 steps: `pnpm dlx syncpack@12 list-mismatches` / `pnpm install --frozen-lockfile` / `pnpm dedupe --check`) appended after the `fitness` job, plus the summary line "25 invariants"→"27". RECONCILED command: `syncpack list-mismatches` (not `pnpm exec syncpack lint` — syncpack is not a workspace dep, and `lint` chokes on the catalog protocol). **GOTCHA for the dedupe step:** verified locally that `pnpm dedupe --check` run IMMEDIATELY after `pnpm install --frozen-lockfile` in the same job emits a FALSE-POSITIVE `ERR_PNPM_DEDUPE_CHECK_ISSUES` exit 1 (pnpm 10.16 quirk — the proposed-changes list is EMPTY and a real `pnpm dedupe` leaves the lockfile byte-identical; the store IS converged). Make the dedupe step resilient: run `pnpm dedupe --check || pnpm dedupe --check` (a single retry; the second run is reliably exit 0), OR run it as the FIRST step before the frozen install. HANDOFF: orchestrator obtains the token, then applies this edit._
- [BLOCKED] **T5.3** Mark `.github/workflows/dependency-updates.yml` `# superseded-by-renovate` (do NOT delete — out of scope, design §5.2). **DoD:** comment present; workflow's `pnpm audit` analysis job retained. File: `.github/workflows/dependency-updates.yml`. _OOM-safe._ _GUARD-BLOCKED — same `.github/workflows/` sensitive-path guard. Edit prepared but NOT applied: add a top-of-file `# superseded-by-renovate (dep-baseline Step 5) — update mechanism replaced by .github/renovate.json; the pnpm audit analysis job is retained as a daily security report.` comment under the existing header. HANDOFF: orchestrator applies with the `sensitive-edit` token._
- [x] **T5.4** Author `.github/renovate.json` (design §5.3): `rangeStrategy: pin`, `pnpm-catalog` manager catalog-awareness, family `groupName`s (react/tiptap/langchain/googleapis/opentelemetry), `minimumReleaseAge: 7 days` (14 for runtime-critical P0). **DoD:** spec scenario "Renovate is configured to pin and group [static]". File: `.github/renovate.json` (new). _OOM-safe._ _DONE — valid JSON; `rangeStrategy: pin`, `pnpm-catalog` manager rule, family groups react/tiptap/langchain/googleapis/opentelemetry + ADDED storybook/testing-library/stryker groups, `minimumReleaseAge: 7 days` global + 14 for the runtime-critical P0 set (@prisma/client, prisma, opossum, prom-client), vulnerabilityAlerts pin. (`.github/renovate.json` is NOT under `.github/workflows/`, so not guard-blocked.)_

### 5.B §7 absorptions + GHSA dated-debt (file edits, OOM-safe)

- [x] **T5.5 — OBSOLETE (closed-by-removal)** `CONCURRENTLY-BUMP` → ~~bump `concurrently` to 10.0.3~~. **SKIPPED:** `concurrently` was REMOVED earlier (Turbo replaced the concurrent dev orchestration — `pnpm dev` = `turbo run dev`). Verified 0 presence in any manifest, the catalog, `pnpm.overrides`, or even transitively in `pnpm-lock.yaml`. Recorded as **CLOSED 2026-06-23 by removal** in `docs/product/PENDING_WORK_INVENTORY.md §7`. _OOM-safe._
- [x] **T5.6** `ESBUILD-OVERRIDE` + `SHELL-QUOTE-OVERRIDE` → keep with dated-debt / remove-when gate (design §2.2 keep-reasons). **DoD:** spec scenario "absorbed standing items carry a gate or are removed [static]" — each surviving override carries a documented remove-when. Files: root `package.json` comments + `docs/product/PENDING_WORK_INVENTORY.md` §7. _OOM-safe._ _DONE — both recorded under §7 "Dated-debt overrides" with explicit remove-whens (esbuild: vite allows ≥0.28.1 + frontends off vite 7; shell-quote: empirical no-op de-dup test). NOTE: kept in `package.json` `pnpm.overrides` as literals (no inline JSON comments possible — JSON forbids comments); the remove-when lives in §7, which is the audit grep's source of truth._
- [x] **T5.7 (Q2 — 3 ignored GHSAs, dated-debt, do NOT move auditConfig)** Convert the `UNDOCUMENTED-GHSA-IGNORES` flag into a dated-debt line per GHSA (`GHSA-q7cg-457f-vx79`, `GHSA-p8p7-x288-28g6`, `GHSA-848j-6mx2-7j84`) with the remove-when from design §6, dated `2026-06-19`. Keep the `pnpm.auditConfig.ignoreGhsas` block in `package.json` (moving to YAML is out of scope). **DoD:** §7 inventory has 3 dated-debt lines with remove-when. File: `docs/product/PENDING_WORK_INVENTORY.md`. _OOM-safe._ _DONE — §7 "Dated-debt GHSA ignores" subsection has all 3 GHSAs dated 2026-06-19 with per-GHSA remove-when. `auditConfig` untouched in package.json._
- [x] **T5.8 (Q2 — SECURITY_CANON note)** Add an "Audited audit-ignores" table to `docs/security/SECURITY_CANON.md` mirroring the 3 GHSAs (reason + remove-when) AND the CVE-floor catalog pins from T1.10 (design §6 + D4). Follow the canon §"How to extend" rules (Owner line + structured section preserved so fitness #24 stays green). **DoD:** table present; fitness #24 (canon child structure) still hard-zero. File: `docs/security/SECURITY_CANON.md`. _OOM-safe._ _DONE — new `## Audited audit-ignores` section added BETWEEN Multi-Tenant Isolation and How to extend, with TWO tables: (1) the 3 ignored GHSAs, (2) CVE-floor pins axios/form-data/validator/ws (catalog) + tough-cookie 4.1.3 + @hono/node-server 1.19.13 (overrides). `**Owner:**` (line 6) + `## How to extend` (line 82) preserved; fitness #24 = hard-zero (PASS)._

### 5.C Spec correction + final gate

- [x] **T5.9** Correct the stale "24 fitness functions" wording in `specs/dependency-version-management/spec.md` (Requirement "The baseline causes no regression…" + its scenario) to **27**, matching `CLAUDE.md §Automated Compliance Checks` and `.github/workflows/fitness.yml`. **DoD:** spec reads "27 fitness functions"; no other count drift. File: `openspec/changes/dependency-baseline/specs/dependency-version-management/spec.md`. _OOM-safe._ _DONE — all 3 "24"→"27" (requirement body + scenario title + scenario step). ALSO RECONCILED the consumer-governed-transitives model (ADR-0018): the "exactly one version repo-wide" requirement re-titled "Every DIRECT registry dependency…" with a model-reconciliation note (single-version governs DIRECT/catalog deps; TRANSITIVES are consumer-governed and may coexist), and every `syncpack lint` scenario reference switched to `syncpack list-mismatches` (the actual CI gate command) with the catalog-protocol-noise rationale._
- [ ] **T5.10 [PAUSE-STACK] — DEFERRED to CI (NOT in this slice)** Final gate: `syncpack list-mismatches` + `pnpm dedupe --check` + `pnpm install --frozen-lockfile` + full build + LXC-safe tests + 27 fitness. **DoD:** ALL spec success criteria green; this is the pinned baseline. _Full build/test on the 9GB box — that is the child PR's CI job, not run locally per LXC-safety. The DETERMINISTIC subset WAS run this slice (all green): `syncpack list-mismatches` exit 0, `--frozen-lockfile` exit 0, `dedupe --check` exit 0 (stable across reruns), `audit --audit-level moderate` exit 0, 27 fitness greps hard-zero._

### 5.D Tracker merge (the only merge to main)

- [ ] **T5.11 — orchestrator (NOT this slice)** After all child PRs (Steps 1–5) are merged into `workstream/dep-baseline` and the tracker is green, open the **single tracker→main PR**. **DoD:** tracker PR green on all CI (the new `dependency-consistency` job + 27 fitness + build/test); atomic rollback boundary preserved (only the tracker touches main). _CI-gated._
