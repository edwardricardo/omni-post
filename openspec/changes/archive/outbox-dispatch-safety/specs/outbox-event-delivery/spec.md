# Delta for outbox-event-delivery

> NEW capability — no existing spec at `openspec/specs/outbox-event-delivery/`. Defines the
> delivery guarantees of the transactional outbox relay (atomic claim/lease, dispatch
> ordering, dedupe, retry/backoff, DLQ terminality), per ARCHITECTURE_CANON §Event-Driven
> Architecture ("after dispatch, mark event PROCESSED atomically"; "every consumer handler
> is idempotent"). Requirements are semantics-decision-agnostic: each holds whether design
> selects Variant A (post-dispatch receipt, atomic with publish) or Variant B (no relay-side
> inbox). RFC 2119 keywords are normative; **[MERGE-BLOCKING]** requirements MUST be proven
> green before merge.
...
undispatched, non-dead-lettered event published. (The current bug violates this: a transient
error after the pre-dispatch inbox write makes the retry short-circuit to
published-without-dispatch and without a dead-letter record.)
...
end in the terminal DLQ state — never in a false published state. (Existing behavior,
