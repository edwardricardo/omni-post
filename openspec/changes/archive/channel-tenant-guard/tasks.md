...
- [x] 10.1 [GREEN] `packages/ports/src/RepoPort.ts` (`:184`): `getChannelsByIds(ids, accountId)` + new `getChannelOwnerAccountId(channelId)` (selects `accountId` column ONLY, never decrypts; JSDoc: "remove with the D2 fallback").
- [x] 10.2 [GREEN] `packages/adapters/db-prisma/src/{ChannelRepository,cached,index}.ts`: `getChannelsByIds` adds `accountId` to `where`, wraps `findMany` in `$transaction` calling `setTenantGuc(tx, accountId)` first; `cached.ts:426-441` cache key becomes `(accountId, ids)` (D9 anti-poisoning); implement `getChannelOwnerAccountId`.
- [x] 10.3 [GREEN] `apps/workers/src/services/CredentialResolver.ts`: `resolve(channelId, accountId: string)`; `publishHandlerTypes.ts:103-105` `CredentialsLookup` signature.
- [x] 10.4 [GREEN] `apps/workers/src/services/ChannelAuthFailureRecorder.ts` (`:49`): `record(channelId, provider, reason, accountId)` — `setTenantGuc` as first statement of the existing `$transaction`; update `where: { id, accountId }` (P2025 on foreign = 404-equivalent).
- [x] 10.5 [GREEN] `apps/workers/src/mentionIngestWorker.ts` (`:137`): `resolveChannelAdapter` calls `setTenantGuc` + scopes by `job.accountId`; call sites `:229,:309` pass `job.accountId`.

## Phase 11: Publish job payload accountId (D2)

- [x] 11.1 [GREEN] `packages/shared/src/saga.ts` (`:609-618`, + `saga.d.ts`): schedule step adds `accountId` to the `"publish-post"` job from `context.metadata.accountId`. **Gate finding #2**: `metadata.accountId` is untyped — narrow to `string` via a type guard / explicit check (throw or skip when absent) — NO `any`, NO `as any` (fitness #3).
- [x] 11.2 [GREEN] `apps/api/src/saga/SagaIntegration.ts` (`:276`): `queueJob` type carries `accountId` (the `...job` spread propagates it; `:439` already populates `metadata.accountId`).
- [x] 11.3 [GREEN] `apps/workers/src/{publishHandlerTypes,publishHandler,publishWorker}.ts`: `payload.accountId?: string`; `publishHandler` computes `eff = payload.accountId ?? getChannelOwnerAccountId(channelId)` once per job, passes a required `string` to `resolve`/`record`. **Bounded fallback — removal condition**: delete the fallback + make the field required once the PUBLISH queue (incl. the BullMQ delayed set — scheduled posts sit for days) holds no pre-deploy jobs.

## Phase 12: RED→GREEN — publish regression (MERGE-BLOCKING)

- [x] 12.1 [GREEN] Worker two-tenant regression (real DB): own-tenant job resolves credentials + publishes green; foreign `(channelId, accountId)` → `err("AUTH")`, nothing decrypted, no reauth flag on B; legacy payload (no `accountId`) → fallback resolves owner + publishes; recorder foreign-scope no-op. Turns 8.1 GREEN. (Authored as `apps/api/tests/integration/publishWorkerTenantIsolation.test.ts` — node:test + real DB, driving the REAL worker collaborators over `createPrismaRepoAdapter` through the same relative-import seam `publish.flow.test.ts` uses. **Not** under `apps/workers/tests/`: CI's Package Tests job runs `turbo run test` over @apps/workers with NO database service, so a DB-backed vitest file there would fail by construction. Wired into the `integration:tenant-isolation` batch of `apps/api/scripts/run-tests.sh`, which CI runs with Postgres. Regression found + fixed en route: `apps/api/tests/publish.flow.test.ts` enqueued legacy payloads and its `PublishRepo` stub lacked `getChannelOwnerAccountId` → `this.repo.getChannelOwnerAccountId is not a function`; the stub now delegates to the real repo and the payloads carry `accountId`.)
- [x] 12.2 Run WVITEST publish-flow + CredentialResolver unit (LXC by-file) + worker INT → GREEN, 0 cancelled.

## Phase 13: Child-table audit + docs + PR2 gate

- [x] 13.1 `docs/security/MULTI_TENANT_GUARDS.md`: RLS policy note + `setTenantGuc` (D3) helper doc + **D10 child-table read confirmation** table (SAFE: `analyticsRoutes` overview/export, `getHistoricalTrends`, admin `groupBy`, hardDelete cascades). **Escalate to backlog** (do NOT silently drop): (1) `getDailySummary`/`getMonthlySummary` latent gap (D10); (2) initiate-time project-ownership probe (D7); (3) NOBYPASSRLS role provisioning + `mentionIngest` Mention-write GUC coverage (D3).
- [x] 13.2 **0-defect gate (PR2)**: `tsc` (@apps/workers, @apps/api, @adapters/db-prisma, @shared/types, @ports/core, @infra/prisma build) = 0; `eslint --max-warnings 0` on touched files = 0; fitness **#3/#8/#9/#10/#21/#23 = 0** (`tenantGuc.ts` outside #23 scope, documented); publish regression (7/7 INT) + worker unit (122/122) + db-prisma (61/61) + apps/api unit (8154/8154) green, 0 cancelled; prettier clean.
- [x] 13.3 **4R review fix-pass** (2026-07-28): B1, B2, C1–C7, W1–W7 + all 3 optional items applied. Highlights: db-prisma tests brought inside the typecheck perimeter (root cause of the arity break, 4 latent type errors fixed); the saga pivot now FAILS CLOSED instead of emitting an unscoped job; the deploy-compat fallback is now observable (`worker_publish_job_account_id_source_total`) and its two failure causes no longer collapse into `AUTH`; `getChannelOwnerAccountId` binds the `__system__` GUC; mention-ingest scope has behavioral tests plus `worker_mention_channel_unresolved_total`; both mention-job producers aligned on the enrolled `Channel.accountId` column.
