...
| 29 | videoProcessor.ts | `analyze-video` | resource (local file, no per-tenant credential) | `cacheEnabled:false`/`fallbackEnabled:false` — uncached (cheap deterministic ffprobe); no public-path cache key | uncached (N-SEC-1 decision) |
...
| C-4 | apps/api/src/trends/trendAnalysisService.ts `:85` | `trend-analysis-service:get-trending-content`             | `cacheEnabled:true` stub w/ NO per-tenant credential (real leak once wired)                  | **SHIPPED: `cacheEnabled:false` + `fallbackEnabled:false`, NO discriminant** — flip-to-false chosen over a synthetic "global" key (see note) | nothing is cached, so no cross-tenant entry can ever be written or served |
| C-4 | trendAnalysisService.ts `:156`                    | `generate-trend-predictions`                              | same                                                                                         | **SHIPPED: `cacheEnabled:false` + `fallbackEnabled:false`, NO discriminant**                                                                 |                                                                           |
| C-4 | trendAnalysisService.ts `:278`                    | `analyze-viral-content`                                   | same                                                                                         | **SHIPPED: `cacheEnabled:false` + `fallbackEnabled:false`, NO discriminant**                                                                 |                                                                           |
| C-4 | trendAnalysisService.ts `:389`                    | `discover-content-opportunities`                          | same                                                                                         | **SHIPPED: `cacheEnabled:false` + `fallbackEnabled:false`, NO discriminant**                                                                 |                                                                           |
...
> **C-4 honesty note (shipped decision).** The four `trendAnalysisService` closures are
> platform-global TikTok-trend stubs with NO per-tenant credential today. Rather than key them by a
> synthetic `"trend-analysis-global"` discriminant, the shipped code sets `cacheEnabled:false` +
> `fallbackEnabled:false` on all four ops and folds NO discriminant — so neither the L1 cache nor the
> L2 fallback store can ever write or serve a cross-tenant entry for them (the strongest posture:
> nothing is cached at all, no discriminant needed). When these are wired to real per-tenant
> provider/`prisma` calls, caching MUST be re-enabled together with a tenant-scoped
> `cacheKeyDiscriminant` as a pair (never cache without the tenant scope) — the shipped in-code
> comment states this contract. Even so, Fix B (D8) means a future omission is an availability, not a
> disclosure, concern. The `apps/`-scoped completeness guard (Fitness #29 idea, design D8) is
> recommended as the follow-up that closes the audit's `apps/` blind spot; it is NOT shipped in this
> remediation.
