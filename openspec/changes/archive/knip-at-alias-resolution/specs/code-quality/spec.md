# Code Quality — Delta Spec

> Delta spec for change `knip-at-alias-resolution`. Capability: **the dead-code gate
> (`Code Quality (knip + jscpd + madge)`) and how knip resolves `@/*` aliases in the Next
> apps**. Source of truth: `proposal.md` + the empirically-verified RCA (a live knip run
> with `oxc-resolver.resolveFileSync` monkeypatched to capture the resolution path; fix
> applied to `knip.json` and validated — `pnpm check:dead-code` → `RATCHET_EXIT=0`).
>
...
>
> This is a delta: it states what MUST be true _after_ the change is applied. It does not
> design the implementation (the exact `knip.json` JSON shape is the design phase's job).
