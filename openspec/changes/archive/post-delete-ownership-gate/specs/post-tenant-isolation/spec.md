...
Capability: `post-tenant-isolation` (NEW). Closes the LIVE cross-tenant IDOR
(CWE-639) on `DELETE /posts/:id` by gating deletion on caller-account ownership
via a REQUIRED caller-context union — closing the same gap the
Update/Archive/HardDelete/Duplicate routes gate, but with a compiler-enforced
(fail-closed) context rather than their optional `callerAccountId?`.

Verification note: the three MERGE-BLOCKING requirements MUST be proven by a
real-DB two-tenant INTEGRATION test through HTTP. A mocked unit test cannot
detect a missing ownership filter.
