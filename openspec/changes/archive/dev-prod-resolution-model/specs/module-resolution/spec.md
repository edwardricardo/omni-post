# Module Resolution — Delta Spec

> Delta spec for change `dev-prod-resolution-model`. Capability: **workspace module
> resolution across dev/test/CI and production**. Source of truth: `proposal.md` +
> the empirically-verified RCA (dist deliberately removed to force the failure mode;
> fix applied; consumers re-resolved with `dist` absent).
>
...
>
> This is a delta: it states what MUST be true _after_ the change is applied. It does
> not design the implementation (the `exports` JSON shapes, the exact flag wiring, and
> the tsconfig edits are the design phase's job).
