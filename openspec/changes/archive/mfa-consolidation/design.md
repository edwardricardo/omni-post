...
**Choice**: New `MfaSubject` type (const-object union): `{ type: "admin" | "customer"; id: string }`. The 5 authenticated customer self-service routes (`/auth/mfa/{status,setup,verify-setup,disable,regenerate-backup-codes}`) build `{ type: "customer", id: request.customerUser.id }`. The `userEmail` bug (`mfaRoutes.ts:99`, id assigned where an email belongs) is fixed at its **source**: `CustomerRequestUser` exposes **no** `email` field (`customerAuthMiddleware.ts:17-24`), so the route cannot pass one; instead `setupMfa` **derives the label email from the `MfaUserRecord.email` it already loads via `findById`**, and the redundant `email` parameter is dropped from `setupMfa`'s signature (admin call sites updated accordingly). Admin routes keep `{ type: "admin" }`. For force-disable parity a NEW route `/admin/customers/:userId/mfa/force-disable` (same `requireAdminAuth` + `USER_MANAGE` guards, audit resource `CustomerUser`) sits beside the existing `/admin/users/:userId/mfa/force-disable` — rejected alternative: one route with a `subjectType` body discriminator (ambiguous audit resource naming, easier to mis-review).

**Tenant isolation**: `customerUser` is in `TENANT_SCOPED_MODELS` (tenantGuard.ts:111), so the customer adapter's typed queries get `accountId` auto-injected when TenantContext is bound. Authenticated customer routes run inside the customer's tenant context — a customer can only touch their own row. Admin-over-customer force-disable is a cross-tenant admin operation → sanctioned `withSystemContext()` path (canon; NO tenant-guard bypass invented).

**Audit `accountId` (S1 from the PR1 verify)**: `MfaService.audit()` passes `subject.id` as BOTH the audit `userId` and `accountId`. That is correct for admin (`AdminUser` is a global table, not in `TENANT_SCOPED_MODELS`; the codebase convention uses the admin id as accountId, cf. `authServiceCore.ts:108`) but WRONG for customer. Fix: `MfaUserRecord` gains an optional `accountId?: string`; the customer adapter returns `customerUser.accountId`, the admin adapter omits it, and `audit()` uses `record.accountId ?? subject.id` for the accountId position (never any secret material).

### Apply-time findings (Explore validation of the Decision-4 assumption — PARTIAL)

The design's pre-auth assumption ("the login verify flow binds tenant context from the identified CustomerUser's `accountId` before the MFA read/write") was validated at apply time and found **PARTIAL**:

- **CUMPLE** for the 5 customer self-service routes: `requireClientAuth` (`customerAuthMiddleware.ts:59-70`) already binds `enterTenantContext({accountId})` and exposes `request.customerUser.accountId`. These 5 routes are safe to repoint to the customer subject in PR2.
- **NO CUMPLE** for the login-time `POST /auth/mfa/verify` (`mfaRoutes.ts:453-457`): it has **no** preHandler, **no** TenantContext, and its body is only `{userId, token}` — no `accountId`. Worse, **the customer login never challenges MFA at all**: `LoginCustomerUseCase` (`packages/core/customer-auth/src/LoginCustomerUseCase.ts`) never checks `mfaEnabled` nor returns `mfaRequired`, and it runs under `withSystemContext`. Building that challenge is a NEW auth flow (challenge token, expiry, anti-replay) that needs its own design.
- Additional ground truth that shapes PR2: `CustomerRequestUser` has **no** `email` field (`customerAuthMiddleware.ts:17-24`) → the `userEmail` fix is source-side derivation, not `request.customerUser.email`. `MfaUserRecord` had **no** `accountId` → added optional for the audit fix.

**Decision (Edward): a NEW slice PR2b** carries the customer login MFA challenge with its own design (challenge token, expiry, anti-replay). **PR2 does NOT touch `/auth/mfa/verify`** — it stays on the admin subject, unchanged, and PR2b owns its repoint + `accountId` resolution.
...
| PR2  | CustomerUser migration + customer adapter + repoint of the **5 self-service** routes (customer subject) + `userEmail` source-fix (drop `email` param, derive from record) + audit `accountId` fix + `/admin/customers/.../force-disable` + no-clobber tests. **Does NOT touch `/auth/mfa/verify`.** | Customer MFA self-service (currently broken-by-design; low regression exposure). Migration additive/non-blocking | Revert + down-migration (drops only the 2 new columns)                              |
| PR2b | Customer login MFA challenge (new auth flow: challenge token, expiry, anti-replay) + repoint of login-time `/auth/mfa/verify` to the customer subject + its `accountId`/TenantContext resolution. Own design.                                                                                       | Customer login path (new challenge, gated behind its own design/verify)                                          | Revert; `/auth/mfa/verify` returns to the admin-subject baseline                    |
