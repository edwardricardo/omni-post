...
- [x] 2b3.1 [RED] (U) `apps/api/tests/unit/RedisMfaChallengeStoreAdapter.test.ts` + port contract: `issue` = `SET NX EX`; `consume` returns `CONSUMED` to exactly one caller (`DEL` count 1), `NOT_FOUND` on second/expired/unknown; store error → typed `err("STORE_ERROR")` (NOT swallowed). Fails RED.
- [x] 2b3.2 [GREEN] Create `packages/ports/src/MfaChallengeStorePort.ts` (`issue`/`consume`, `MfaChallengeStoreError`) + `packages/ports/src/MfaVerificationPort.ts` (move `MfaVerificationResult` + `MfaVerifyTokenError` here); export both in `packages/ports/src/index.ts`. JSDoc `@layer domain`.
- [x] 2b3.3 [RED→GREEN] (Uc) `packages/core/customer-auth/tests/unit/challengeBinding.test.ts` + create `packages/core/customer-auth/src/challengeBinding.ts` (`sha256` hex helper, node crypto). `@layer application`.
- [x] 2b3.4 [RED] (U) `apps/api/tests/unit/customerJwt.test.ts` — token-kind confusion BOTH directions: a `customer-mfa-challenge` token rejected by `verifyCustomerToken`/refresh verify (aud + type); an access/refresh token rejected by `verifyCustomerMfaChallengeToken`; alg `["HS256"]`/iss `omnipost-customer`/aud `omnipost-customer-mfa` pinned. Fails RED.
- [x] 2b3.5 [GREEN] `packages/core/domain/src/repositories/CustomerTokenService.ts`: add `CUSTOMER_MFA_CHALLENGE_TTL_SECONDS = 180`, `CustomerMfaChallengeClaims`, `signMfaChallengeToken`/`verifyMfaChallengeToken` signatures. `@layer domain`.

**Use cases (non-sensitive; fake ports)**

- [x] 2b3.6 [RED] (Uc) extend `packages/core/customer-auth/tests/unit/LoginCustomerUseCase.test.ts` — **no session minted pre-MFA**: `mfaEnabled` → returns `CustomerMfaChallengeOutput` (not tokens), performs NO `recordLogin`/`save`/`recordSuccessfulAttempt`/mint; `store.issue` failure → `MFA_UNAVAILABLE`; customer **WITHOUT** MFA → unchanged token mint. Fails RED (no branch today).
- [x] 2b3.7 [GREEN] `packages/core/customer-auth/src/LoginCustomerUseCase.ts`: branch on `targetUser.mfaEnabled` after `isActive`+rehash; hash `iph`/`uah`; `store.issue(jti, TTL)`; sign challenge; widen return to `Result<LoginCustomerOutput | CustomerMfaChallengeOutput, …>`; add `MFA_UNAVAILABLE`; append `MfaChallengeStorePort` dep. 2b3.6 GREEN.
- [x] 2b3.8 [RED] (Uc) `packages/core/customer-auth/tests/unit/CompleteCustomerMfaLoginUseCase.test.ts` — step-2 anchors: happy TOTP+backup mint session; **two concurrent valid step-2 → exactly ONE session** (jti consume serializes; loser → `INVALID_CHALLENGE`); wrong code → challenge NOT consumed + `recordFailedAttempt({failureReason:"MFA_FAILED"})` + retry succeeds; **BF success recorded only post-MFA**; expired/consumed/foreign challenge + binding mismatch → **byte-identical `INVALID_CHALLENGE`** (anti-oracle); `consume` store error → `MFA_UNAVAILABLE`. Fails RED (file absent).
- [x] 2b3.9 [GREEN] Create `packages/core/customer-auth/src/CompleteCustomerMfaLoginUseCase.ts` (deps incl. `MfaVerificationPort`, `MfaChallengeStorePort`, `BruteForceProtectionPort`, `UnitOfWork?` LAST): verify JWT → binding → load by `sub` → isActive/mfaEnabled → BF check → `verifyMfaToken` → consume `jti` → UoW `recordLogin`+`save` → `recordSuccessfulAttempt` (outside tx) → mint. Export use case + DTOs in `packages/core/customer-auth/src/index.ts`.

**Adapters + JWT + routes [Window C for the sensitive files]**

- [x] 2b3.10 [GREEN] Create `apps/api/src/infrastructure/adapters/RedisMfaChallengeStoreAdapter.ts` (dedicated `createRedisConnection()`, keyPrefix `auth:mfa-challenge:`, `SET NX EX`/`DEL`-count, typed fail-closed errors). `@layer infrastructure`. 2b3.1 GREEN.
- [x] 2b3.11 [SENSITIVE][GREEN][Window C] `apps/api/src/auth/customerJwt.ts`: 4th token kind — payload `type:"customer-mfa-challenge"`, sign+verify pin alg `["HS256"]`/iss `omnipost-customer`/aud `omnipost-customer-mfa`, carry `jti`+`iph`+`uah`. 2b3.4 GREEN.
- [x] 2b3.12 [GREEN] `apps/api/src/infrastructure/adapters/CustomerTokenServiceAdapter.ts`: delegate the 2 new methods to `customerJwt.ts` (follows 2b3.11).
- [x] 2b3.13 [SENSITIVE][GREEN][Window C] `apps/api/src/admin/auth/MfaService.ts`: add `implements MfaVerificationPort`; import `MfaVerificationResult`/error union from `@ports/core` (drop local copies). Additive on top of PR2b-1's TOTP edit — do not revert it.
- [x] 2b3.14 [RED] (U) extend `apps/api/tests/unit/customerAuthUseCases.test.ts` + route inject: step-2 route maps errors per the design taxonomy (401 generic / 403 / 429 / **503 `MFA_UNAVAILABLE`** / 500); **store outage → 503 + WARN `threat_type:"mfa_challenge_store_unavailable"`** (fail-closed — deliberate contrast to the limiter's fail-open); binding mismatch → WARN `mfa_challenge_binding_mismatch`; `resolveClientIp` (NEVER `request.ip`) feeds `ip` on both handlers. Fails RED.
- [x] 2b3.15 [SENSITIVE][GREEN][Window C] `apps/api/src/auth/customerAuthRoutes.ts`: login handler challenge mapping + `resolveClientIp`; NEW `POST /auth/customer/login/mfa` route/handler/`MfaLoginSchema`/`errorMap` under `withSystemContext("customer-mfa-login", …)`; resolve `TOKENS.CompleteCustomerMfaLoginUseCase`. 2b3.14 GREEN.
- [x] 2b3.16 [SENSITIVE][GREEN][Window C] `apps/api/src/security/httpRateLimitPreHandler.ts`: add explicit `{ path:"/auth/customer/login/mfa", config: RateLimitConfigs.AUTH }` to `AUTH_ROUTE_RULES`; update the `:122-126` comment (rule stays — covers `/auth/mfa/verify-setup`). (U) `authRateLimit` pattern (`:212-230`): step-2 6th request 429s.

**DI wiring (non-sensitive)**

- [x] 2b3.17 [GREEN] `apps/api/src/infrastructure/container/types.ts`: add `TOKENS.MfaChallengeStore` + `TOKENS.CompleteCustomerMfaLoginUseCase` (2 new tokens ONLY — no token for `MfaVerificationPort`).
- [x] 2b3.18 [GREEN] `setupServices.ts`: register `RedisMfaChallengeStoreAdapter` under `TOKENS.MfaChallengeStore` (dedicated `createRedisConnection()`, `:839-855` pattern). `setupCustomerAuthUseCases.ts`: `LoginCustomerUseCase` gains the store dep; register `CompleteCustomerMfaLoginUseCase` resolving `TOKENS.MfaService` **typed as `MfaVerificationPort`** (reuse, not a 2nd token — fitness #21).

**Orphan retirement + cleanup [sensitive edit in Window C; isolate as its own commit]**

- [x] 2b3.19 [SENSITIVE][GREEN][Window C] `apps/api/src/auth/mfaRoutes.ts`: DELETE the orphan `POST /auth/mfa/verify` — registration `:523-528`, handler `:191-233`, `MfaVerifySchema` `:41-46`. (U) inject: `POST /auth/mfa/verify` → **404**.
- [x] 2b3.20 [GREEN] Cleanup blast radius: delete 3 orphan tests `tests/unit/mfaRoutes.test.ts:341-369`; retarget `tests/unit/authRateLimit.test.ts:212-230` to `/auth/mfa/verify-setup`; regenerate `packages/shared/src/api-generated/*` (dump:openapi flow — retires `types.gen.ts:431`); update `docs/api/auth.md:123`, `docs/api/README.md:131`, `docs/architecture/API.md:161` (replace orphan row with the new endpoint); fix/remove k6 helper `performance/k6/utils/auth-helpers.js:201`.

**Integration + gate**

- [x] 2b3.21 [RED→GREEN] (I) `apps/api/tests/integration/customerLoginMfa.integration.test.ts` — real DB+Redis, single file: enroll → login → challenge → complete → mints a token pair; two parallel `consume` on real Redis → exactly one `CONSUMED`. `pnpm db:up` first.
- [x] 2b3.22 Regression: PR1/PR2 MFA suites + `tests/mfa.test.ts` (21) + admin login suites green; e2e MFA spec (`apps/client/tests/e2e/tests/auth.spec.ts:231-254`) expectations revisited at apply.
- [x] 2b3.23 0-defect gate — `eslint --max-warnings 0`, `tsc` clean, LXC-safe single-file runs green, `size:exception` in PR body (~1,600 > 400; orphan-retirement isolated commit). Fitness hard-zero (exact greps):
...
- [x] 3.1 RED (node:test): seed legacy admin rows; backfill moves codes exactly once; re-run is no-op; source `passwordResetToken` retained until verified; guard skips genuine reset tokens; assert fitness #23 count `0` (typed Prisma only). — DONE (non-sensitive pass): `tests/integration/backfillAdminMfaBackupCodes.integration.test.ts` (282 lines, 7 `it()` blocks) pins the `runBackfill`/`verifyIntegrity`/`runCleanup` contract (each takes an injected `PrismaClient`). Fails RED for the RIGHT reason: `ERR_MODULE_NOT_FOUND` at `infra/prisma/scripts/backfill-admin-mfa-backup-codes.js` (script absent), NOT a syntax error. Covers: legacy-blob → migrated once + source retained; idempotent re-run; UUID-v4 reset token skipped; `CHANGE_REQUIRED` sentinel skipped; already-has-codes skip-migrated; verify counts source-matching vs migrated; cleanup nulls only guard-matched+codes-present, pending reset token never nulled.
- [x] 3.2 GREEN (sensitive pass): created `infra/prisma/scripts/backfill-admin-mfa-backup-codes.ts` (271 lines, NOT `scripts/migrations/` — the real data-migration precedent is `infra/prisma/scripts/migrate-team-member-to-customer-user.ts`). Keyset-batched `findMany` (`id: { gt: lastId }`, `orderBy id asc`, take 200 — robust to filter-mutation, no infinite loop on skipped rows); pure `parseLegacyBackupBlob` guard (`startsWith("[")` + `JSON.parse` in try/catch + every element `startsWith("$argon2id$")`, non-empty array); skip-migrated (`mfaBackupCodes: { isEmpty: true }` at query level); per-row typed `update`; source `passwordResetToken` retained. `// canon-exception: migration:2026-07-11` (full valid scenario). JSDoc `@layer infrastructure` tags-first. Import-safe: CLI `main()` guarded by `isDirectRun()` (`import.meta.url === pathToFileURL(process.argv[1]).href`) — importing never connects/exits. Zero raw SQL (fitness #23 not in scope + 0 anyway). RED → GREEN 7/7, 0 cancelled.
- [x] 3.3 GREEN (sensitive pass): `verifyIntegrity(prisma)` returns `{ sourceMatching, migrated }` (counts guard-matching sources while retained; `migrated <= sourceMatching` invariant proven by test). Separate `runCleanup(prisma)` returns `{ cleaned }`, nulls `passwordResetToken` ONLY where the guard matched AND `mfaBackupCodes` non-empty — a pending UUID reset token and the `CHANGE_REQUIRED` sentinel never satisfy the guard (don't start with `[`), so never nulled (both asserted GREEN). Default CLI `main()` runs backfill→verify only; cleanup gated behind `--cleanup` so source retention is the safe default.
- [x] 3.4 DONE (sensitive pass): deleted `apps/api/src/auth/mfaService.ts` (498 lines) — file ABSENT. Pre-deletion safety net confirmed ZERO real import statements of `auth/mfaService` anywhere in `apps/api/src` + `packages/*/src` (all 4 former prod sites already resolve the unified `admin/auth/MfaService.js` via DI per PR1/PR2 ground truth; no repoint needed). Test-side was already done in the non-sensitive pass: legacy contract test `tests/unit/mfaService.test.ts` (590 lines, 27 legacy-only assertions) DELETED — parity intact in `tests/mfa.test.ts` (21/21) + `unifiedMfaService.test.ts` (19/19). `tsc --noEmit` @apps/api = 0 after deletion (nothing referenced the file).
- [x] 3.5 DONE (sensitive pass): zero-reference gate CLEAN — `rg "from '...auth/mfaService'"` → ZERO real imports (repo-wide, excl. reports/generated/stryker); file ABSENT. 3 benign historical "why" doc comments remain (NOT imports, all correct post-deletion): `tests/mfa.test.ts:26`, the backfill RED's own JSDoc `tests/integration/backfillAdminMfaBackupCodes.integration.test.ts:9`, and `setupServices.ts:190` (documents the closed single-registration invariant). TOTP single-use (PR2b-1) intact — `unifiedMfaService` 19/19 + `test:mfa` 21/21 green. Fitness #23 = 0 (apps/api/src + apps/workers/src; the new script under `infra/prisma/scripts/` is out of #23's scope and has 0 raw queries). Test repoints were done in the non-sensitive pass (15 DI-bootstrap-only tests → unified ctor, 8 static / 7 await-import).
