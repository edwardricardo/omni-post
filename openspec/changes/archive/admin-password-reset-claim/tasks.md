...
- [x] **0.1** Stage the change directory `openspec/changes/admin-password-reset-claim/` — `explore.md`, `pre-propose-decisions.md`, `proposal.md`, `specs/*/spec.md` (×3), `design.md`, `tasks.md`, `evidence/probe-reset-claim-sql-v3.mts`, `evidence/capture-output-v3.txt`. Measured: 2,724 lines already on disk + this file.
- [x] **0.2** (N9) Include the **uncommitted** `openspec/config.yaml` staleness fix already in the working tree — `:21`, `:62`, `:86` carrying 27 → 40. This is the ORCHESTRATOR's earlier correction, not this change's 40 → 41 bump; sequencing it here is what lets PR 2 present a clean three-line 40 → 41 diff instead of a 27 → 41 diff that reads as this change's doing.
- [x] **0.3** Apply the three spec amendments the design states verbatim under "Spec amendments the orchestrator applies" — gate baseline "8 marker sites = 7 claim sites + 1 named issuance exception"; residual-limits bullet 4 "line-window matching" → "argument slicing"; refresh spec non-goal gains "…and the per-mint `jti` the rotation claim requires to be a claim (design D9)". **Verify before editing**: all three amendments are ALREADY present in the spec files on disk (`single-use-claim-gate/spec.md:104-109` and `:198-201`, `admin-refresh-rotation-claim/spec.md:45-46`). If so, this item is a confirmation, not an edit — record it as confirmed and change nothing.
- [x] **0.4** No code, no test, no gate run in this unit. It exists so the planning trail is in the PR a reviewer reads.
...
- [x] **2.1** RED: add the D9 uniqueness case to `apps/api/tests/unit/authService.test.ts` — two `generateTokens` calls for one payload **in the same tick** (no await between them) → assert distinct `refreshToken` strings, distinct `hashRefreshToken` values, and `jwt.decode(...).jti` a UUID on each. On `19fb9e6a` this is RED **deterministically** (identical strings). That suite does not mock `jsonwebtoken`, so the real signer runs. Record the RED.
- [x] **2.2** `apps/api/src/auth/authServiceCore.ts` — extend the `crypto` import to `import { randomBytes, randomUUID } from "crypto";` (`:9`) and add `jwtid: randomUUID(),` to the **refresh** `jwt.sign` options (`:427-433`) with the design's site comment. The ACCESS token is deliberately left alone: nothing consumes it by a write, so uniqueness there would be symmetry without a requirement. ~8 lines.
- [x] **2.3** `apps/api/src/auth/authTypes.ts` — `jti?: string;` on `TokenPayload` (1 line, registered claim, present on refresh tokens).
- [x] **2.4** (N8) The site comment and the PR body state D9's rationale on its own mechanism. **Do not cite `customerJwt.ts:53` as precedent** — that `jti` is a different token (an MFA challenge id), so the cite would be false. D9's soundness does not rest on a precedent.
- [x] **2.5** (DF-7) Delete `apps/api/tests/auth.test.ts:286-287` — the comment `// Wait 1 second to ensure new JWT has different iat timestamp` and `await new Promise((resolve) => setTimeout(resolve, 1000));`. The `notEqual` assertion that follows becomes the **no-sleep integration pin** of D9. That sleep is the tree's own record of the defect; deleting it converts a timing dependence into a proof.
- [x] **2.6** Blast-radius re-verification before GREEN is claimed (the design verified it; confirm it still holds): the refresh JWT is parsed in `apps/api/src` only at `authServiceSession.ts:82` (`jwt.verify` → cast to `TokenPayload`) and `:238` (`jwt.decode` for `exp`); `apps/admin` decodes no JWT. An extra registered claim is ignored by both; the token grows by ~45 bytes inside an opaque cookie/header.
- [x] **2.7** GREEN: the D9 case and the de-slept `auth.test.ts` both pass; run `auth.test.ts` as a single heap-capped file.
...
- [x] **4.1** Insert the #41 block into `CLAUDE.md` after `:1486`, inside the existing fence, with one blank separator. Paste the design's `design.md:344-526` verbatim. Update `:305`: "There are **40 checks, numbered #1-#40**" → "**41 checks, numbered #1-#41**".
- [x] **4.2** (N5) Add ONE clause to the NAMED EXCEPTION paragraph, alongside the existing remove-when: the exemption keys on **file + column**, not on the caller, while its justification is a property of the CALLERS. It is sound today because `updateRefreshTokenHash` has exactly ONE caller tree-wide (`createSession`) — verify that caller set at apply and state the verification. A second caller that consumed a presented hash would inherit the exemption unseen. ~2-3 comment lines; they are inside the mirrored body, so they cost double (section 9).
- [x] **4.3** Run the block from the repo root on the post-U3 tree → **`0`**. If it is not 0, STOP: either a fix is incomplete or the marker list is wrong, and the gate must not be tuned to fit a broken tree.
- [x] **4.4** **Red-proof protocol** (the repo rule: a new gate ships with its red demonstrated — plant, real non-zero exit, byte-exact restore, re-confirm 0). Use the scratchpad for the originals; `cp`/`cmp` only, **never git**. Six plants:
...
- [x] **4.5** (N6) **Correct expectation for step 2 of each plant.** The `CLAUDE.md` copy ends with `echo "$COUNT"`, so it prints **only the count** — the violation LINES live in `$VIOLATIONS` and are never echoed there. Expect stdout `1` and nothing more. To see `apps/api/src/admin/auth/PasswordService.ts:<n>: passwordResetToken consumed in data but not named in where`, run the **yml step body** (which does `printf "%s\n" "$VIOLATIONS"`) or inspect `$VIOLATIONS` directly. The design's red-proof step 2 expects the violation line from the CLAUDE.md copy; it will not appear there, and a proof written against the wrong expectation reads as a failed plant.
- [x] **4.6** Mirror into `.github/workflows/fitness.yml` as a new step after #40 (`:1452`), before `Fitness summary` (`:1454`), under **ONE** `omnipost-allow sensitive-edit` token (15-minute window, audited in `.claude/heuristic-overrides.log`). If the `CLAUDE.md` edit also trips the pre-edit blocker, the SAME token covers it — do not request a second. The step body is the block re-indented 10 spaces, with the yml tail: `Fitness #41: $COUNT violations (expect 0)`, then `if [ "$COUNT" -gt 0 ]` → one `::error title=Fitness #41 violation::…` + `printf` + `exit 1` + `fi`. Every `::error` pairs with an `exit 1` in the SAME step (fitness #34). Update the summary at `:1459`: "40 architectural invariants (#1-#40)" → 41 / `#1-#41`.
- [x] **4.7** **Drift check** (gate spec: a `diff` of the extracted blocks is empty): extract `sed -n "/^# 41\./,/^EOF$/p"` from `CLAUDE.md` and the same range from the yml step, strip leading whitespace per line, `diff` → empty. Paste, never paraphrase.
- [x] **4.8** `openspec/config.yaml` 40 → 41 at `:21`, `:62`, `:86` — three lines, in this commit (N8). The 27 → 40 correction already landed in U0, so this diff reads as exactly what this change did.
- [x] **4.9** `docs/security/SECURITY_CANON.md` — extend the "How to extend" companion-checks line with `#41` single-use claim shape (one line).
- [x] **4.10** (Reconciled at archive, 2026-09-15: this archive executor has no shell/Bash tool and cannot re-run the fitness suite itself. Reconciled on: (a) the orchestrator's launch-prompt final-state fact that PR #255, commit `8ec9f4a7`, merged the #41 gate to main `12bca1c0`; (b) static repo evidence read directly — `CLAUDE.md:305` reads "41 checks, numbered #1-#41" on the current tree, not 40; (c) this repo's branch-protection/CI convention requires the fitness workflow green before a merge lands. This is NOT an independent re-run of the suite — flagged honestly in the archive report as a merge-fact-based reconciliation, not a fresh measurement.) Re-run the full fitness suite on the merged shape: **41 checks, #41 at 0, no other ratchet raised** (#30 still at or below 21; #38's db-prisma sub-count still at or below 11).
- [x] **4.11** (Reconciled at archive, 2026-09-15 — same reconciliation basis as 3.16/4.10: PR #255 is merged per the orchestrator's final-state fact; this writer has no GitHub read tool to inspect the PR2 body directly.) PR 2 body carries the rollback order (gate reverts FIRST; the `fitness.yml` revert consumes a second sensitive-edit token), the measured population ("8 marker sites = 7 claims + 1 named issuance exception"), the red-proof evidence, and the dependency diagram with the current PR marked.
...
- [x] **5.1** `docs/reports/roadmap-detected-smells-backlog.md` — SMELL-97 row → **DONE** with PR numbers and the merge sha.
- [x] **5.2** Open the rows. Re-read the file at archive time (2026-09-15): next free number was confirmed **still 110** (no concurrent close moved it — SMELL-109 was the last row). Opened **SMELL-110 through SMELL-118** (9 rows: the 6 named in this table + DF-6 + DF-2 + the gate-pass-2 second-admin-refresh-flow finding).
...
- [x] **5.3** `docs/product/MASTER_PLAN_ES.md` — dashboard §6 (Nota cell for row N, "40 checks"→"41 checks" + closure mention) + the [CRED] `admin-password-reset-claim` (SMELL-97) ficha, with verified PR numbers and shas.
- [x] **5.4** DONE 2026-09-15 by the orchestrator (promoted the 3 NEW-capability specs via cp + `diff -r` byte-identical; archived the change folder via `git mv`). The original archive executor's toolset (Read/Edit/Write/Glob/mem_*) has no shell/Bash tool. Per SKILL.md's Mechanical Copy Contract, archiving the change folder (`git mv`) and promoting the three NEW-capability delta specs (`cp` + `diff -r`, byte-identical, never Read→Write) are shell-only operations with no Read/Write fallback permitted. Same precedent as `password-reset-integrity` and `analytics-route-port-integrity`. Remaining work for a follow-up run with shell access, listed in the archive report's "Orchestrator TODO" section.
...
**Band re-adjudication (Edward, 2026-09-13, post-U1 measurement):** U1 alone measured EVIDENCE 1,631 (racer 577 + fake 571 + unit tier 475 + wiring 8; ledger 1,876 with CODE) — the writer stopped without trimming per the ritual. RATIFIED as measured: the forecast undercounted against its own named precedents (the model fake is 487 lines for ONE delegate/23 columns; U1's covers TWO delegates/46 columns and is proportionally leaner). **New whole-change band ~2,000-2,150, writer hard stop ~2,470.** Ledger reset executed with this ratification as reason (revision `23937be7…`). Recalibration rule recorded for every future forecast: **a harness line that cites a precedent MEASURES that precedent first.**
...
**Final adjudication (Edward, 2026-09-13, at PR-1 readiness):** after the 4R fan-out (4/4 APPROVE-WITH-NOTES, zero blockers) and the single adjudicated polish unit U3b (14 items; ledger 546/600), the measured totals were presented and BOTH ratified:

1. **EVIDENCE 2,643 measured** vs the re-adjudicated stop 2,470 (+173) — RATIFIED; drivers itemized in the PR body. Recalibration #9 recorded: a multi-item polish writer keeps a RUNNING per-item estimate — a single settle-time measurement cannot see the stop before crossing it.
2. **PR 1 CODE 422 changed** (338+/84−) vs the 400 hard budget — **size exception RATIFIED** (the ask-on-risk decision): the +22 is the observability hardening the 4R lenses required pre-merge; additions-only reads 338.
