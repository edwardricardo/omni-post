...
| `packages/shared/src/saga.ts`                                                        | Modify        | S1+S2 | `SagaInstance` gains optional `updatedAt` in S1 (C1 consumes it in `checkSagaTimeout`'s COMPENSATING branch); `SagaStepResult` union REBUILD, step classes emit `outcome`, wait step returns `waiting` in S2 (D6)                                                                                                                                                                         |
