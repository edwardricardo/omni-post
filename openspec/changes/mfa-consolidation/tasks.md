...
- [ ] 2b2.1 [RED] (Uf) extend `apps/client/lib/auth/__tests__/authApi.test.ts`: `authApi.completeMfaLogin({challengeToken, code, rememberMe})` POSTs proxy `/auth/customer/login/mfa`; `login` maps `data.mfaRequired` → `MfaChallenge {requiresMfa, message, methods, challengeToken, expiresInSeconds}`. Fails RED.
- [ ] 2b2.2 [GREEN] `apps/client/lib/auth/authApi.ts`: add `challengeToken: string; expiresInSeconds: number` to `MfaChallenge`; add `completeMfaLogin`; map fields at `:129-135`. 2b2.1 GREEN.
- [ ] 2b2.3 [GREEN] `apps/client/lib/auth/authContext.tsx`: `login` returns `Promise<MfaChallenge | null>` (was throw `:111-115`); add `completeMfaLogin` context method. Register-auto-login `null` path preserved.
- [ ] 2b2.4 [RED] (Uf) proxy test for `app/api/backend/[...path]/route.ts`: a step-2 response through `auth/customer/login/mfa` persists `httpOnly` session/refresh cookies and **does NOT leak tokens to the browser**; step-1 challenge body still passes through (no cookies). Fails RED (exact-path match `:51`/`:135` today).
- [ ] 2b2.5 [GREEN] `apps/client/app/api/backend/[...path]/route.ts`: add `AUTH_LOGIN_MFA_PATH = "auth/customer/login/mfa"` to the cookie-persist branch (`:135`) + `parseRememberMe` (`:99-101`). 2b2.4 GREEN.
- [ ] 2b2.6 [RED] (Uf) `apps/client/components/auth/MfaChallengeForm.test.tsx` (+ `page.tsx` two-step) — RTL: password step ⇄ challenge step; wrong-code (`INVALID_MFA_CODE`) keeps challenge; invalid-challenge/503 falls back to password step with message; `completeMfaLoginAction` request shape includes the XFF relay. Fails RED.
- [ ] 2b2.7 [GREEN] Create `apps/client/components/auth/MfaChallengeForm.tsx` (`@component` + `@layer infrastructure`; props documented on the interface; function order per REACT_STANDARDS; challenge in `useActionState`/hidden input — memory/DOM only, never storage). Wire `apps/client/app/[locale]/login/page.tsx` two-step render. 2b2.6 GREEN.
- [ ] 2b2.8 [GREEN] `apps/client/app/actions/auth.ts`: `loginAction` returns `{mfaChallenge:{challengeToken, expiresInSeconds, rememberMe}}` on `data.mfaRequired` (fixes the `:80-82` dead end); NEW `completeMfaLoginAction` POSTs `/auth/customer/login/mfa` with the `forwardedForHeaders` relay (`:62-67`), persists via `setSessionCookie`/`setRefreshCookie({rememberMe})`, `redirect` to dashboard.
- [ ] 2b2.9 [GREEN] i18n: add `auth.*` MFA-step keys to `apps/client/messages/en.json` + `es.json` (es neutral professional Spanish).
- [ ] 2b2.10 0-defect gate: `eslint --max-warnings 0`, `tsc` clean; fitness #9/#10/#12 (`@component` on `MfaChallengeForm`) + #17 (no new `process.env` in client) hard-zero; `size:exception` rationale (~800 > 400). Inert: no user-visible change until PR2b-3 emits `mfaRequired`.
