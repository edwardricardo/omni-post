...
#### A4-S6 — §2F Slice 6: narrow double-post window via durable provider-receipt persistence (WRK-DOUBLE-POST core) — DONE (on committed Slices 1-3 HEAD bd968112, strict RED-first TDD, no git ops)

> WRK-DOUBLE-POST core. A publish job that crashes AFTER the provider accepts the
> post but BEFORE the OK log commits re-runs on BullMQ retry and PUBLISHES THE SAME
> POST AGAIN (X/most providers have no native idempotency key). The "claim + confirm
> receipt" pattern persists the provider post id on a durable COLUMN immediately
> after provider success (before the OK log), and a retry guard confirms that receipt
> instead of re-publishing. Honest residual: NARROWS the provider-commit → receipt-
> commit window, does NOT eliminate it (exactly-once = provider-native idempotency,
> Slice 7, deferred). Step 1 (nullable `providerPostId String?` column + migration
> `20260629045507_add_publish_log_provider_post_id`) was applied by the orchestrator
> (security-gated path); this slice built on that column with no schema/migration edit.

- [x] S6.1 SCHEMA (orchestrator, security-gated): nullable `providerPostId String?` on `PublishLog` + migration `20260629045507_add_publish_log_provider_post_id` (ADD COLUMN nullable TEXT) applied; Prisma client regenerated to expose the field. No app-side schema/migration edits in this slice (gated paths).
- [x] S6.2 PORT + ADAPTER: added `providerPostId: string | null` to `PublishLog` (RepoPort.ts) + `recordReceipt(dedupeKey, providerPostId): Promise<Result<void, "DATABASE_ERROR">>` to `RepoPort`; implemented in `PublishLogRepository` as a typed `prisma.publishLog.update({ where:{dedupeKey}, data:{providerPostId} })` (NOT raw — fitness #23 safe; transitively tenant-scoped like `logPublish`); mapped the column through `logPublish`/`getLogByDedupeKey`/`listLogs`; added the write-through delegate in `cached.ts`.
- [x] S6.3 WORKER TYPES: widened `PublishRepo.getLogByDedupeKey` to `{ status: string; providerPostId?: string | null } | null` + added `recordReceipt`; updated `createMockRepo` (setup.ts).
- [x] S6.4 HANDLER (strict RED-first): RED recorded — on a non-ERR RUNNING row carrying a receipt, the retry RE-PUBLISHED (provider called 1×, expected 0) because the only guard was `status==="OK"` and no receipt was written pre-OK. GREEN: (a) retry guard at top of `handleJob` — a NON-ERR row with a non-null `providerPostId` → confirm receipt, skip provider (keeps the `status==="OK"` short-circuit + thread THREAD_EXISTS idempotency; ERR rows still re-publish); (b) `recordReceipt(dedupeKey, res.value.providerPostId)` called in `publishSinglePost` IMMEDIATELY after provider success, BEFORE the OK log; (c) thread path records the root tweet id as the durable receipt right after `publishThread` success, before the OK log. Honest-residual JSDoc on the column (schema), `recordReceipt` (RepoPort + adapter), and both handler call sites + the retry guard.
- [x] S6.5 0-DEFECT GATE (touched-file, LXC-safe): new `publishHandler.doublePost.test.ts` 3/3 (RED→GREEN: provider called 0× on retry; ERR still re-publishes; receipt written before OK log); db-prisma `subRepos.di` 14/14 (+recordReceipt typed-update test); regression jobHandler 15, edgeCases 9, single 11, thread 14, reauth 6, db-prisma resilience+adapter 45. lint `--max-warnings 0` (heap 4096) = 0 on all 8 touched files. typecheck single run (heap 6144) = 0 across `@ports/core` + `@adapters/db-prisma` + `@apps/workers`. fitness #3 (no-any) / #7 (dedupeKey determinism untouched) / #8 (no phase refs) / #9 (@file) / #10 (@layer) / #23 (no raw queries) = 0. NO git ops, NO schema/migration edits. Detail in apply-progress SLICE-6.
- [x] S6.6 DUAL-JUDGE FIX (F1 BLOCKING + F2 LOW): F1 — the api flow test's hand-rolled `createPublishRepoFromCtx` wrapper (`apps/api/tests/publish.flow.test.ts`) was missing the now-required `recordReceipt`, breaking `@apps/api` tsc (tests in tsconfig) + the happy-path runtime (`this.repo.recordReceipt` undefined). Added a `recordReceipt` delegate to the underlying `ctx.repo` (`createPrismaRepoAdapter` already implements it via `RepoPort`, so a real delegate — no stub) and widened the stale `getLogByDedupeKey` cast to include `providerPostId?: string | null`. F2 — both `recordReceipt` call sites in `publishHandler.ts` discarded the `Result`; narrowed `.ok` and emit a handler-logger WARN on failure (honest-residual: degrade to the documented re-publish window, never fail the publish over a receipt-write miss). GATE: `@apps/api` typecheck (heap 6144) = 0; `publish.flow.test.ts` 5/5 (0 cancelled) targeted via the `flow` batch invocation; worker `publishHandler.doublePost.test.ts` 3/3 still green; lint (heap 4096) = 0 on both touched files. NO git ops, NO schema/migration edits.
