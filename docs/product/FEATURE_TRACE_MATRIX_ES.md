...
| Miembros por proyecto con roles granulares | Agencia       | 🟡 PARCIAL      | `ProjectMember` `schema.prisma:369` (join projectId+memberId+`permissions[]`) — modelo presente, **sin cablear**: 0 lecturas/escrituras en prod, acceso hoy account-wide vía tenant guard; roles son account-level (`CustomerRole`), no por-proyecto. Distinto de "RBAC/roles" (este nivel, account-level) y "Dashboards/espacios por cliente" (Nivel 6, multi-tenant). Gap: falta enforcement + write path + rol-por-proyecto. |
...
| 2FA / seguridad de cuenta          | Enterprise  | ✅ IMPLEMENTADO | `schema.prisma:3131` (`require2FA`), `MfaManager.tsx`. **Consolidado 2026-07 (PRs #129-133, N-SEC-5):** `MfaService` unificado subject-typed detrás de `MfaUserRepositoryPort` + adapters Prisma admin+customer; persistencia MFA de customer; TOTP single-use (claim atómico) + backup-codes CAS single-use; **gate de MFA en el login de customer** end-to-end (challenge en Redis 180s + ruta step-2 + UI cliente). Legacy `auth/mfaService.ts` eliminado. Specs vivos: `openspec/specs/{unified-mfa-service-and-port,customer-mfa-persistence,mfa-flow-correctness,customer-login-mfa-challenge}`. |
