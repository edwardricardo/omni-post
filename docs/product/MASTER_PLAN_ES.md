...
- **CQRSIntegration dead-route** `[S]` (P-VERIFY-2 bonus) — **RESUELTO:** `apps/api/src/cqrs/CQRSIntegration.ts` se confirmó DEAD_CODE (nunca cableado en ningún composition root) y se eliminó del repo (branch `workstream/cqrs-integration-purge`); la clasificación pendiente queda cerrada. FN-009 (optimizedPostsRoutes) + FN-010/027 (rateLimitingDashboard) siguen abiertos, sin cambios.
