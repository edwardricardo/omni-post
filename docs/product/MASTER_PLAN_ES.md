...
| N.B Correctness del core-publishing         | 7      | 1      | 🔄     |
| N.C Red de regresión (assurance)            | 3      | 0      | ⬜     |
| N.D Docs P1                                 | 2      | 0      | ⬜     |
| **Total Nivelación**                        | **17** | **4**  | 🔄     |
...
- [ ] **N-SEC-3** `[M]` **API+INFRA** — IDOR-TRACKEDLINK + guard para modelos `projectId`-only. `confirmed-adversarial` (ALTA) + root-cause `ARCH-PROJECT-SCOPED-GUARD-GAP`. `apps/api/src/links/linkRoutes.ts` get/stats/**delete** por `:id` sin gate de accountId; `TrackedLink` no tiene columna `accountId` → fuera del guard. **DoD:** gate de ownership en las rutas de links; y un mecanismo (guard secundario o denormalización de `accountId`) para los ~9 modelos `projectId`-only (ProjectMember, Post, **Channel** [credenciales cifradas], TrackedLink, Campaign, ScheduledReport, ExternalNotificationConfig, GeneratedImage, RecurringPost) — priorizar Channel y ExternalNotificationConfig por contener secretos descifrables. Verificar el patrón compensatorio de join documentado en `MULTI_TENANT_GUARDS.md §Transitively-scoped` está presente en cada adapter (el audit S2.1d no lo evidenció). **🔄 EN CURSO (2026-07-28)** — rollout secuencial en marcha, **Approach A** (denormalizar `accountId` + guard + RLS, por construcción; OWASP API1:2023). Clasificación de los 9 modelos hecha (`openspec/changes/project-scoped-tenant-guard/rollout-plan.md` + engram obs 260): **7 de 9 con IDOR cross-tenant VIVO**. **Cerrado hasta ahora:** (a) IDOR read/delete de TrackedLink → PRs #109/#110 (adapter-join + purga CQRS); (b) hotfix `DELETE /posts/:id` (Slice 0) → #112; (c) **reference impl** ExternalNotificationConfig al guard+RLS (Slice 1, uno de los 2 credential-bearing priorizados) → #113; (d) ScheduledReport+Campaign al guard+RLS (Slice 2) → #114 — incluye el cierre del **untag join-table IDOR** (`campaignPost` sin `accountId`: el enrolamiento no cubre mutaciones de tabla join; fix app-level con `findById` guardado) y, colateral, el arreglo del CI para branches stacked (`workstream/**` triggers) + el fix del boot de Integration Tests que estaba ROJO en todos los pushes a `main` desde 2026-06-24. Todos con full SDD (8 fases) + gate adversarial + verify PASS + CI verde; (e) RecurringPost+TrackedLink al guard+RLS (Slice 3, fold estructural) → #116; (f) GeneratedImage al guard+RLS (Slice 4) → #118; (g) ProjectMember al guard+RLS (Slice 5, forgotten-feature enrolado defensivamente) → #119; (h) **Channel** al guard+RLS + tenant-safety de los paths worker (Slice 7, el modelo credential-bearing de máximo blast: 4 columnas AES-GCM descifradas por el credential-resolver) — **PR1 #152** (columna `accountId` + backfill + flip del guard 57→58 + ownership en los dos create paths + par RLS por la suite de paridad guard↔RLS) + **PR2 #164** (scoping explícito por `accountId` + `setTenantGuc` en los 4 sitios worker, `accountId` en el payload del job de publish con productor fail-closed, regresión publish two-tenant), **VERIFICADO** (PASS 0C/5W/3S) **+ ARCHIVADO** — cierra además el **IDOR de credenciales del lado worker** (los paths que leían `Channel` en el cliente RAW sin scope de tenant: `CredentialResolver`, `ChannelAuthFailureRecorder`, `mentionIngestWorker`, más el owner-lookup del fallback de deploy). **Enrolamiento estructural: 8/9 modelos** (ExternalNotificationConfig, ScheduledReport, Campaign, RecurringPost, TrackedLink, GeneratedImage, ProjectMember, Channel) — falta solo **Post**. **Eje de contexto pre-auth (propagación del `TenantContext` en los bordes, ADR-0020):** (i) `tenant-context-preauth-seams` (Slice 6.1, Class A — SSO SAML/OIDC, integration-auth Zapier/Make, webhooks billing/inbound, health, OAuth-flow, index bootstrap) **VERIFICADO** (PASS 0C/2W/3S) **+ ARCHIVADO** → #120; el seam de integration-auth aterrizó como **split de dos hooks** (`integrationAuthResolve` onRequest + `integrationAuthBind` preHandler), superior al holder mutable que describía el plan original (reconciliado en el archive). `tenant-context-coverage-harness` (Slice 6.0, ratchet de cobertura) tiene proposal+design (fase tasks pendiente). **Restante:** slice **Post** (Slice 8 — último modelo del enrolamiento; cierre estructural del gate app-level de `post-tenant-isolation`) · `tenant-context-coverage-harness` tasks+apply · audit de callers fuera de contexto · `api-guarded-client-injection` (swap del adapter bootstrap `index.ts:301` + fitness #28; activa el scoping cross-tenant de A6 health). **Hallazgos de ejecución (no vistos en la valoración):** (1) el create-path de cada modelo acepta `projectId` del cliente sin ownership check (engram obs 273) → paso obligatorio de la receta; (2) el enrolamiento NO cierra rutas que mutan o atraviesan tablas join/hijas ni las que saltan el `findById` guardado — aplica a WRITEs y READs (engram obs 285) → cada slice audita la superficie completa de rutas y su suite two-tenant cubre TODAS las rutas del modelo. Backlog SMELL-54/55/56.
...
- [ ] **N-COR-2** `[M]` **API(saga-runtime)** — Sagas in-flight mueren en deploy/restart + loop de re-fail + wait-starvation. `confirmed-adversarial` (ALTA ×3). (a) `loadActiveSagas()` en boot no reanima PENDING/RUNNING → el timeout las FAILEA sin compensar; (b) `failSaga` no hace `activeInstances.delete` + timeout checker sin filtro terminal → re-fail cada 60s para siempre (EventStore crece sin cota); (c) `WaitForPublishingCompletion` quema los 3 retries con "still pending" → publicaciones lentas/multi-canal mueren FAILED. **DoD:** reanudar PENDING/RUNNING(sin nextRetryAt) en `initialize()`; `activeInstances.delete` + guard terminal en `failSaga`/timeout; separar "waiting" de "failed" en el contrato del step (pending no consume retryCount). **Sub-ítem (a) CERRADO por `saga-tenant-scope-and-recovery` (N-COR-2a) — ver ficha N-COR-7 abajo.** **Sub-ítem (b) CERRADO también en `main` por ese mismo change**, no diferido: `failSaga` da de baja la saga del set en memoria por el mismo helper `stopTracking` que usan las rutas de completion y compensación (`apps/api/src/saga/SagaManagerExecution.ts:526`) y el timeout checker se niega a re-visitar una fila terminal (`apps/api/src/saga/SagaManagerLifecycle.ts:1127`), así que el re-fail cada 60 s y el crecimiento sin cota del EventStore ya no ocurren. Dejaron de ser diferibles en cuanto el parking creó una fila no-terminal que espera a propósito. **Queda abierto (c)** — `WaitForPublishingCompletion` quema el presupuesto de reintentos con "still pending" — junto con la amplificación de `handleEvent` y el in-flight guard, en el change sucesor `saga-engine-terminal-hygiene` (change 2 de N.B).
...
- [x] **N-COR-7 ✅ + N-COR-2a ✅** `[M]` **API(saga)+INFRA** — `SagaInstance.accountId` = userId + persistencia saga bypasea el tenant guard + loops de recuperación muertos. `confirmed-adversarial` (ALTA). **CERRADO** (SDD `saga-tenant-scope-and-recovery`, chained PRs stacked-to-main: **PR1 #173** merged `55d659af` (2026-08-05) + **PR2 #180** merged a `main` `bd1adbb4` (2026-08-11); **VERIFICADO** PASS-WITH-WARNINGS — 17/17 requisitos, 46/46 escenarios COMPLIANT, 0 CRITICAL (engram `sdd/saga-tenant-scope-and-recovery/verify-report`, obs 456) — **+ ARCHIVADO**). El hallazgo original ("engine usa el `prisma` crudo") resultó ser la causa correcta pero por un mecanismo distinto al hipotetizado: NO existe bypass general de `$transaction` (refutado empíricamente en diseño) — el bootstrap simplemente nunca pasaba el cliente guardeado a `SagaIntegration` (`index.ts:687` resolvía el singleton crudo). Alcance real shipeado: `SagaContext.accountId` de primera clase (nunca más `userId` en la columna) · migración de backfill con 4 disposiciones (metadata-first / join / sentinel-terminal / RAISE-no-terminal) · motor completo en el cliente guardeado con contexto declarado dual-capa (`sagaTenant.ts`: lectura system-scoped query-scoped para boot/scan/by-id, escritura bajo tenant REHIDRATADO por saga) · **N-COR-2a** (boot resume de un solo pase + retry-checker ahora vivo, ownership partition por `nextRetryAt`) · fallo terminal honesto vía `failSagaAsSystem` (única escritura cross-tenant del motor) · lifecycle de PARKING para sagas interrumpidas en/tras el pivot (auto-resume evaluado y GATEADO-Y-RECHAZADO empíricamente: el replay del pivot se absorbe pero el paso post-pivot pierde por OCC, así que el parking es la postura normativa, con condición de revisita documentada) · orden de composición corregido (definitions ANTES de `initialize()`) · boot acotado y contenido (`bootLoadLimit`, `maxConcurrentSagas`, contención por fila y por pase) · observabilidad real (10 contadores `SagaMetrics` + Prometheus + alertas) · vocabulary fix (`parked` ≠ el drain de shutdown, que HANDS OFF). Living specs: `openspec/specs/{multi-tenant-isolation,tenant-context-boundaries,saga-crash-recovery}/spec.md`. Detalle: `openspec/changes/archive/saga-tenant-scope-and-recovery/archive-report.md`. **Carried a `saga-engine-terminal-hygiene` (change 2 de N.B):** el warning de propagación del exit code del runner (`run-tests.sh` captura el código por batch pero no lo propaga al exit del script para el caso 0-fail/0-cancel), las reclamaciones **SMELL-73** (single-replica; row claims vía `OutboxClaimService`), `parkedAt` no durable across restart (per-proceso, re-abre la ventana en cada reinicio), la starvation de `take:50` en el retry-scan (head-of-line, misma página re-seleccionada cada tick), el resto de la higiene terminal (waiting≠failed, in-flight guard, `handleEvent` amplification — la baja de `activeInstances` y el filtro terminal del checker NO se cargaron: se PROMOVIERON dentro de este mismo change y cierran N-COR-2(b) en `main`), y el dedupe durable de CQRSBus (**SMELL-71** duplicate DRAFT en replay de `create` · **SMELL-72** registro muerto `TOKENS.SagaManager`).
...
- [ ] **DOCS-INDEX-REBUILD** `[S]` **DOCS** — el `docs/_DOCS_INVENTORY.md` de 2026-05-10 fue ELIMINADO en el slice de limpieza Pre-Fase: era el input de adjudicación ya consumido y, al ejecutarse los borrados, sus links quedaron rotos (regla lychee). **DoD:** generar el inventario fresco contra el árbol actual (metodología en `docs/audits/AUDIT_METHODOLOGY.md`) + barrido de links rotos residuales en `docs/` (repuntar o quitar).
...
| **N — Nivelación** (bloqueante) | 17                | 4      | 🔄     | N-SEC-1/1b·2·5 cerrados (campaña 07-19..21, PRs #124/#125/#129-133); N-COR-7+N-COR-2a cerrados (`saga-tenant-scope-and-recovery`, PRs #173/#180); ver §1 |
...
Este plan **reemplaza** como fuente única de planificación a: `IMPLEMENTATION_PLAN_ES.md` (spine de features → §2-4, **retirado 2026-06-29**), `PENDING_WORK_INVENTORY.md` (consolidación → §1/§5, **congelado 2026-07-21** en `docs/archive/`, su contenido único migrado a `SECURITY_CANON §Audited audit-ignores` + `roadmap-detected-smells-backlog.md`); y **se apoya en / deriva de** la valoración `FULL_REPO_ASSESSMENT_2026-06-29.md` (**CONSERVADA** como companion de evidencia → §1/§5). Rescata como referencia viva: `FEATURE_TRACE_MATRIX §9` (canon 2026 → §0.4/§2-4), `PLAN_MAESTRO §5.8/§5.9` (disciplina → §0.3).
