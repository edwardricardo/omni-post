...
- **WRK-DOUBLE-POST** — crash between provider-OK and OK-log re-posts the tweet (idempotency gap). **Provenance:** WF2 `wrk-publish-double-post-02` (`known_smell`). **STATUS 2026-06-29: CONFIRMED + NARROWED (in #97).** Closed the crash window with a durable `PublishLog.providerPostId` receipt (written immediately after provider success, BEFORE the OK log) + a receipt-aware retry guard — Slice 6, commit `eb295f40`. Honest residual: provider-commit→receipt-commit is not atomic, so this NARROWS (at-least-once, tighter window) not eliminates; exactly-once needs provider-native idempotency keys (Slice 7, DEFERRED).
- **OAUTH-REFRESH-UNWIRED** — no OAuth token refresh in publish flow; `OAuthTokenRefresher` unwired; double-refresh race. **Blocks F1-API-4 (Canva OAuth) confidence.** **Provenance:** WF2 `oauth-refresh-01` (ALTA) + MEDIA. **STATUS 2026-06-29: CONFIRMED, DEFERRED.** The AUTH-signal classifiers (Slice 1, in #97) are the prerequisite; the actual refresh wiring is DEFERRED (Slices 4-5): OAuthTokenRefresher transient-vs-definitive (flag reauth only on `invalid_grant`, not 5xx/network), the X `bearerToken`-vs-`accessToken` disconnect (refresh has ZERO effect on X publish today), TikTok v2 URL+body-auth, and REMOVE `facebook` from `TOKEN_URLS` (false-flags reauth every 60-day expiry). **Still blocks F1-API-4 (Canva).** Map: workflow `wcx4ozdvc`; v1 scaffolding in `git stash@{0}`.
- **WRK-NO-REAUTH** — publish worker doesn't set `needsReauth` on cred failure → channel silently broken. **Provenance:** WF2 `wrk-publish-no-reauth-04` (ALTA). **STATUS 2026-06-29: CONFIRMED + FIXED (in #97).** All four publish AUTH sites (single + thread, credential + provider) now flag `needsReauth` + emit `ChannelAuthFailed` via the canonical `handleProviderAuthError` — Slice 3, commit `bd968112`. Prerequisite: Slice-1 classifiers (facebook/tiktok/youtube/threads/bluesky mapped every auth error to NETWORK so AUTH never fired) + Slice-2 false-positive mappers (telegram/x 403, snapchat invalid_token) — both in #97.
- **§2F-DEFERRED (follow-up workstream)** — the dedicated exploration (workflow `wcx4ozdvc`) proved §2F is a ~7-slice cross-provider workstream, not 3 patches. The critical subset (Slices 1-3 + 6) landed in #97. **DEFERRED to a follow-up:** Slice 4 (refresh transient-vs-definitive), Slice 5 (X/TikTok refresh wiring + facebook `TOKEN_URLS` removal — see OAUTH-REFRESH-UNWIRED), Slice 7 (per-provider idempotency — **bluesky is NOT idempotent today** despite atproto supporting `putRecord`+deterministic `rkey`; meta `creation_id` reuse; snapchat deterministic names; pinterest CB-retry-safe). Smaller backlog: FB `makeApiRequest` read-path 401 (no `error.code` body), X 429 numeric `.code`→NETWORK, X `publishThread` 4xx via `readHttpStatus`. The SAGA-ID/dedupeKey lead was adversarially **REFUTED** (deterministic `cmd-${sagaId}-${stepId}`, terminal-state guard, outbox SELECT…SKIP LOCKED all present; fitness #7 = 0).
