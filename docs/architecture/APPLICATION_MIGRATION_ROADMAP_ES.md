...
| **A6.6**  | `TokenService` (jwt-algorithm canon) + `CredentialPort` + logger | customer-auth COMPLETO (6) + team/InviteTeamMember (7). Su email (A6.4) y password (A6.5) ya están a port; restan customerJwt (`TokenService`, jwt-algorithm canon), PlatformCredentialService (`CredentialPort`), lib/logger (logger inyectado). Mayor riesgo.                                                                                                                                                                                   | Alta        | PENDING        |
