...
| **A7**    | Burn-down shims                                                             | Migrar import-sites de application restantes → `@core/application`; borrar shims; (se une al P8 del dominio + flip dependency-cruiser a error). **DONE (fe710c4) = P8a**: 20 barrels @core + 238 import-sites repointeados + `apps/api/src/application` borrado (242 shims). Restan P8b (domain) + P8c (flip depcruise).                                                                                                                                                                                                                                     | Media       | DONE (fe710c4) |
...
en vivo; veredicto = fijar `algorithms:["HS256"]` en verify (HS256 simétrico se mantiene; emisor=verificador único).
Registrado en `canon_research_index.md`.
