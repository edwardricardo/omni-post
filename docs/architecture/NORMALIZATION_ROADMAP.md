...
### 6.1 Containerization image hardening — `P1` · `M` · `STATUS: IN-PROGRESS (PAUSADO 2026-07 — pending bundler ADR-0017; branch `containerization-image-hardening` conservada, PR #89 draft sin merge + docker bumps #98-101 deferidos con él)`
...
| 2026-07-21 | **Campaña de merge/extracción 2026-07-19..21** (registro cruzado; el cluster de seguridad se trackea como Nivelación **N-SEC-\*** en `docs/product/MASTER_PLAN_ES.md §1`, no como items de este roadmap). Aterrizaron en `main` desde las mega-branches cerradas (#97/#107/#108/#110): N-SEC-1/1b cache cross-tenant del circuit-breaker (#124), N-SEC-2 resolver de client-IP + `TRUSTED_PROXY_HOP_COUNT` + fitness #28/#29 (#125), CWE-269 register público removido (#126), higiene (#127), purga CQRSIntegration + re-baseline del floor de cobertura 55→54 (#128, ver §2.2), N-SEC-5 consolidación MFA completa (#129-133) + fundación audit-actor A1/ADR-0020 (#130). Batch de dependencias/higiene (#134-141): codeql-action v4, 2 olas CVE (brace-expansion/axios/js-yaml/protobufjs → `SECURITY_CANON §CVE-floor pins`), cierre F-1 ioredis `commandTimeout:0` (#136), catalog slices testing/code-quality (eslint held en 9 → SMELL-66), `S3_ENDPOINT`. §6.1 containerization sigue PAUSADO (bundler ADR-0017 primero). Reporte: `docs/reports/merge-campaign-2026-07-21.md`. |
