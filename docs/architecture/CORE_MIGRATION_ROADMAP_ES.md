...
| **P8** | Burn-down shims + gate hard | Migrar import-sites restantes → `@core`; **borrar todos los shims**; flip `dependency-cruiser` a **error** (CI gate). Dividido: **P8a application DONE (fe710c4)** — 20 barrels @core + 238 import-sites repointeados + `apps/api/src/application` borrado (242 shims). Restan **P8b** (domain: `apps/api/src/domain`, 136 shims) + **P8c** (flip depcruise core-\* a error). | Media       | P3–P7      | IN-PROGRESS (P8a DONE fe710c4)         |
