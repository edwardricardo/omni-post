...
| **P8** | Burn-down shims + gate hard | Migrar import-sites restantes → `@core`; **borrar todos los shims**; flip `dependency-cruiser` a **error** (CI gate). Dividido: **P8a application DONE (fe710c4)** (20 barrels @core + 238 import-sites + `apps/api/src/application` borrado, 242 shims); **P8b domain DONE (6ef962c)** (barrel VO @core + 333 import-sites incl. 2 `import()` dinámicos + `apps/api/src/domain` borrado, 143 shims) — **andamiaje strangler 100% removido de apps/api**. Resta **P8c** (flip depcruise core-\* a error). | Media       | P3–P7      | IN-PROGRESS (P8a+P8b DONE; resta P8c)  |
