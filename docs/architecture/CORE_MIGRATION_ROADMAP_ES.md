...
| ------ | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------- | ------- |
| **P0** | Scaffold + boundary gate    | Crear `@core/domain` + `@core/application` (package.json, tsconfig, alias en `tsconfig.base`, project refs, barrels vacíos). `dependency-cruiser` reglas de frontera en **warn**. Sin mover código.                                       | Baja        | —          | PENDING |
| **P1** | Kernel base (con shims)     | A `@core/domain`: `Entity`, `AggregateRoot`, `EntityId`+IDs base, `errors/`, `events/DomainEvent` (reconciliar dup con `@shared`), `repositories/Repository.ts` (UnitOfWork). A `@core/application`: `UseCase.ts`. Shims en rutas viejas. | Media       | P0         | PENDING |
