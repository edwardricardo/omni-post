...
| **P9** | Cablear deployables         | apps/workers resuelve use-cases de `@core`; mover driven adapters Prisma a `packages/adapters/*` **donde haga falta** (enfoque **A-rigurosa** bajo convención, §2bis). **P9.0** convención de adapters canónicos (0086df4); **FN-004** autoRenewal: worker duplicado borrado, SoT=api con BackgroundTaskScheduler (0718334); **P9c** roto no-circular SMELL-42 (0c9a861); **P9b** `mentionIngest`→@core: `Mention` aggregate + `MentionRepository` port + `IngestMentionUseCase` + `PrismaMentionRepository` (db-prisma) + worker cableado. `publish` queda worker-only (ejecuta lo que @core agenda; no hay `PublishPostUseCase`). `ChannelAuthFailureRecorder`→backlog SMELL-44. | Media       | P8         | DONE (P9.0·FN-004·P9c·P9b)                     |
