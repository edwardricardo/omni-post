...
| `DELETION_NAME_DIGEST_ACTIVE_VERSION` | Optional with default (`1`); `z.coerce.number().int().min(1)`        | Which key-ring generation NEW name digests are pinned to. Not a secret and not catalogued. Existing rows keep their own pin and are verified under it, so bumping this never reaches backwards. Boot refuses a value naming no ring entry. |
...
**What the value now does.** The daily `deletion-record-degrader` tick enforces the horizon: once `retainUntil` passes, the plaintext `name` is replaced by a keyed HMAC digest in one guarded write, so this number bounds the ACTUAL retention and not merely the justification. The `deletion_record_overdue_plaintext` gauge is the standing witness — it drains to 0 on a healthy population, and a level that stops draining names rows the sweep could not process (typically a name holding a codepoint the pinned Unicode version leaves unassigned, which is flagged and left intact by design rather than digested into a value nobody could ever verify).

**`DELETION_NAME_DIGEST_KEY_RING` — the key behind that digest.** A REQUIRED secret with no default (CWE-798): one JSON object of generation → 64 lowercase hex characters, validated at boot. It is **append-only** — versions must be contiguous from 1, so removing a generation refuses boot by naming the missing version. It is deliberately NOT `PLATFORM_ENCRYPTION_KEY`: that key wraps data and rotates by re-wrapping, while this one authenticates names and can never re-wrap anything, because the plaintext it digested is gone by design. Custody, generation and the rotation runbook live in [docs/security/SECRETS.md §3a](../security/SECRETS.md).

> **Quote the ring value with SINGLE quotes in every `.env*` file.** These files have two readers that disagree, measured rather than assumed: `apps/api/scripts/run-tests.sh` SOURCES the root env file with bash when `DATABASE_URL` is unset, and bash strips the inner double quotes from an unquoted `{"1":"…"}`, yielding `{1:…}` — not JSON, so the app refuses to boot with an error that names the ring rather than the quoting. A double-quoted-and-escaped value has the mirror defect: bash reads it correctly while dotenv keeps the backslashes. Only `'{"1":"…"}'` survives both.
