...
Each of the 61 tenant-scoped tables gets this policy:
...
## Tenant-scoped tables (62 models — `accountId` directly on row)
...
Channel                     Post                    PostContent
PostMedia                   PostChannelPublication
```
...
**`CreatePostUseCase` (Slice 1b) — the recipe applied to a path that was LIVE-exposed.**
The use case validated only that `projectId` parsed as a UUID; it never resolved the
project and never checked ownership, so a caller bound to tenant A could persist a post
into tenant B's project. Reachable through four surfaces that pass a caller-supplied
`projectId` straight through: `POST /zapier/actions/create-draft`,
`POST /zapier/actions/schedule-post`, and the two matching `make` actions. Closed by
resolving the project through `PostRepository.findProjectOwnerAccountId` — which reads it
under the bound context, so a foreign project is invisible and answers `null`, the same
answer a nonexistent one gives — and returning `NOT_FOUND` before any aggregate is built.
All four routes map `NOT_FOUND → 404`. `CreatePostFromRecurrenceUseCase` carries the same
gate; its `projectId` is server-derived from the recurrence's ownership chain, so the gate
is there to give an unresolvable project the right SHAPE rather than to stop a foreign one.
`DuplicatePostsBatchUseCase` needs no new gate: it takes its `projectId` from the SOURCE
post, which `filterIdsByAccount` has already restricted to the caller's own rows.

The composite foreign key underneath is a BACKSTOP, and the distinction matters: a create
that only the database refuses has already lost the ability to answer the caller properly
— it surfaces as a 500 with an engine message rather than the 404 the recipe requires.
...
**`Post`, `PostContent` and `PostMedia` LEFT this category.** They carry `accountId`
directly now and are enrolled in the guard — see §"Post trio promotion" below. The
example above is kept as the shape a transitively-scoped table still needs; for the
trio itself the correct filter is the local column, and the composite foreign key is
what makes the two forms provably equivalent rather than merely intended to agree.
...
### Post trio promotion — `Post`, `PostContent`, `PostMedia` (58 → 61)

The trio's tenancy used to live one table away: `Post` reached its account through
`Project`, and the two children reached it through `Post`. That is why they sat in
§"Transitively-scoped tables" and why layer 1 could not speak for them — there was no
column to inject. Reads had to remember to join, and the two access paths that skip the
parent entirely (`PostContent`/`PostMedia` by `postId`) had nothing at all standing in
front of them.

They now carry `accountId` on the row, tied to the parent by a COMPOSITE foreign key
rather than by convention. That is the difference between "the child agrees with its
parent" being a rule the code follows and being a value the database cannot hold:

```sql
FOREIGN KEY ("projectId", "accountId") REFERENCES "Project"(id, "accountId")
  ON DELETE CASCADE          -- owned-child convention
                             -- ON UPDATE is NO ACTION (PostgreSQL omits the default
                             -- clause; confupdtype='a' in pg_constraint)
```

`ON UPDATE NO ACTION` is load-bearing and is NOT the Prisma default. Under the default
`CASCADE`, re-pointing a project's `accountId` would silently rewrite every descendant's
tenant key — a whole subtree changing hands with no write to it. Under `NO ACTION`
PostgreSQL refuses the parent update while children reference it.

**3-step enrollment applied (canon checklist):** (1) appended `post`, `postContent` and
`postMedia` to the `TENANT_SCOPED_MODELS` Set in
`infra/prisma/src/extensions/tenantGuard.ts` (58 → 61), with the non-null `accountId`
columns, the TOTAL `(id, accountId)` uniques on `Project` and `Post` that the foreign
keys reference, and the composite keys themselves, staged across six migrations
`20260909000000` → `20260909000500` (`20260527000000` untouched); (2) the three
`tenant_isolation` policies via `20260909000500_add_rls_post_trio`, copied verbatim from
the base migration's shape; (3) documented here.

The uniques are TOTAL, deliberately: PostgreSQL cannot reference a PARTIAL unique index,
and a filtered one would strand children whose parent has been soft-deleted.

`VALIDATE CONSTRAINT` ran and PASSED on all three, so the guarantee covers rows that
already existed — not only new writes. A `NOT VALID` constraint would have been
forward-only and must never be reported as covering history.
...
#### Named residual — three collection reads that reach posts without a scope

The scope-first conversion covers EIGHT members of the trio's collection surface. Three are
deliberately NOT converted, and they are named here rather than left in a task annotation
because a residual recorded only in a change record is a residual nobody finds again:

| Member                                         | Why it carries no scope                                         |
| ---------------------------------------------- | --------------------------------------------------------------- |
| `findByStatus(status, pagination?)`            | cross-tenant sweep by shape — no project of its own             |
| `findReadyForPublishing(limit?)`               | the publish sweep; a required scope would misstate what it does |
| `findWithFilters(filters, pagination?, sort?)` | straddles both shapes — its `projectId` filter is OPTIONAL      |

`findWithFilters({ projectId })` therefore reaches exactly the rows
`findByProjectId(scope, projectId)` reaches, and it compiles without naming a tenant.

**This is a compile-time gap, not a runtime one, and the distinction was measured rather
than asserted.** On the application role, under tenant A's bound context:
`findWithFilters({ projectId: B })` returns **0 rows** (control: the same call under B's
context returns 1 row), and `findByStatus("DRAFT")` returns **0 rows** and does not contain
B's post (same control). Layer 1 injects `accountId` from the bound context for every read
through the guarded client, and `20260909000500`'s policy re-checks it in the engine. What
is absent is the third layer the other eight members have — a signature a wrong call cannot
be written against.

**Owner: Slice 2 / the FK slices.** The honest fix is a SPLIT (a scope-first per-project
form beside a sweep that names its cross-tenant intent), not a signature bump, and a split
needs the surrounding surface settled. Tracked as **SMELL-92** in
`docs/reports/roadmap-detected-smells-backlog.md`; **SMELL-91** owns the other half, because
`apps/api/tests/**` sits outside every tsconfig project, so a compile-time pin placed in an
ordinary suite is inert until that scope is widened.

#### Per-table index exemption for `PostContent` / `PostMedia`, with its revisit trigger

`Post` gets ONE new index, `@@index([accountId, projectId], where: { deletedAt: null })`.
The two children get NONE, and that is a decision rather than an omission: their reads
are parent-key-led (`postId`, or `(postId, locale, revision)`), measured at 0.011–0.090 ms
on index or index-only scans in the pre-migration capture recorded in
`docs/reports/TENANT_RLS_AB_MEASUREMENT.md` §Before. An `accountId`-led index on either
child would be write cost paid against a read shape nothing performs.

**Revisit trigger, verbatim:** if either child's post-migration `EXPLAIN` shows a
sequential scan induced by the RLS policy qual, the `accountId`-led index is added in the
...
each one gets its tenant. The rule throughout: the value is READ from the parent row, never
accepted from a caller.

| Write path                                                     | Tenant source                                                                                   |
| -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `PrismaPostRepository.doCreate` (post + content + media)       | `Project.accountId`, resolved in the same transaction through the guarded client                |
| `PrismaPostRepository.doUpdate` (content upsert, media upsert) | the `accountId` returned by the `Post` update itself                                            |
| `PrismaApproveVariantAdapter.createDraftPost`                  | the resolved `Project` row's `accountId`                                                        |
| `packages/adapters/db-prisma` `createPost` (worker-side)       | `Project.accountId`, read inside the transaction                                                |
| `packages/adapters/db-prisma` `addMediaToPost`                 | the parent `Post` row's `accountId`                                                             |
| Nested relation creates (`contents: { create: … }`)            | supplied by Prisma from the composite relation — a nested child cannot name a tenant of its own |
| `infra/prisma/seed.ts`                                         | writes no trio rows (verified, zero matches)                                                    |
| `scripts/rls-ab-measurement.ts` (evidence harness)             | the seeded tenant id, identical for parent and child by construction                            |

Application-level create paths (`CreatePostUseCase`, `DuplicatePostsBatchUseCase`,
`CreatePostFromRecurrenceUseCase`) never supply a tenant at all — they assert that the
target project resolves inside the caller's scope and let the repository derive it. See
§"Create-path parent-ownership".

### `PostChannelPublication` enrollment — the per-channel publication record (61 → 62)

A NEW `accountId`-bearing child of `Post`, one row per `(post, channel)`, holding the
per-channel outcome of a publish. It is born enrolled rather than promoted: the table and
its enrollment land in the same change, so there is no window in which it exists
unguarded.

It takes the trio's shape, for the trio's reason:

```sql
FOREIGN KEY ("postId", "accountId") REFERENCES "Post"(id, "accountId")
  ON DELETE CASCADE
  ON UPDATE NO ACTION        -- NOT the Prisma default; see the trio section above
```

`accountId` is therefore not a value the row may disagree with its parent about — it is a
value the database refuses to hold. That is what closes the create-path shape the guard
alone cannot: layer 1 injects `accountId` from the bound context but never checks the
parent, so without the composite key a tenant could persist a row carrying its OWN account
against a FOREIGN `postId`.

**3-step enrollment applied (canon checklist):** (1) `postChannelPublication` appended to
the `TENANT_SCOPED_MODELS` Set in `infra/prisma/src/extensions/tenantGuard.ts` (61 → 62);
(2) `ENABLE ROW LEVEL SECURITY` + the `tenant_isolation` policy in
`20260917093257_add_post_channel_publication`, in the canonical InitPlan-wrapped form of
`20260910000000_rls_initplan_post_trio`; (3) documented here and in the model list above.

**All three enrollment legs are carried in full.** Leg 1 is not merely the column: the row
also carries an `Account` relation and an `accountId`-led composite index
(`@@index([accountId, postId])`), because this record IS read account-led — the operational
sweeps and the account-wide reads of the publication surface bind the tenant and not a
post. The `PostContent` / `PostMedia` index exemption below does NOT extend to it, and the
difference is the access pattern, not a preference.

**The `channelId` foreign key is `NO ACTION`**, where every other child of `Channel`
cascades. Deleting a channel must not silently shrink a post's recorded target set: the
record would then no longer say what was attempted, which is the one question it exists to
answer. `NO ACTION` is checked at end of statement, so an account-wide cascade that removes
the posts as well still succeeds.

**Enforcement** is `apps/api/tests/integration/postChannelPublicationTenantIsolation.test.ts`,
in the `integration:tenant-isolation` batch of `apps/api/scripts/run-tests.sh`, plus the
`pg_catalog` coverage gate in `rls-tenant-isolation.test.ts`, which enumerates
`getTenantScopedModels()` and therefore covers this table by construction from the moment
it is enrolled.
