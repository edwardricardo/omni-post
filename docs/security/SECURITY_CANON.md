...
| `brace-expansion`                  | `1.1.16` / `2.1.2` / `5.0.7` | `overrides` (3 range-scoped literals, `pnpm-workspace.yaml`) | TRANSITIVE CVE floor (GHSA-3jxr-9vmj-r5cp, high, exponential-time DoS — one advisory spanning all three major lines). Range-scoped so each coexisting major stays at its minimal patch per ADR-0018; validated 2026-07-20 (`pnpm audit` surfaced all three ranges)                                                                                             |
