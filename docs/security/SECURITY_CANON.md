...
| `GHSA-mh99-v99m-4gvg` | `brace-expansion` 1.x/2.x lines (unbounded-expansion DoS) | high     | Added 2026-07-28. The 3.x/5.x line IS patched (override at 5.0.8). The advisory's `<=5.0.7` range also covers the 1.x/2.x lines in semver terms and upstream published NO backport — 5.0.8 is the only patched release. Forcing every line to 5.0.8 was tried and **reverted**: brace-expansion 5 dropped the callable default export the eslint toolchain consumes via `minimatch ^3.1.2`, so eslint dies with `TypeError: expand is not a function` (empirically reproduced). Residual instances are dev-only lint/depcheck tooling parsing our own glob patterns, never request input | eslint's plugin chain moves off `minimatch` 3             |
...
| `brace-expansion`                  | `1.1.16` / `2.1.2` / `5.0.8` | `overrides` (3 range-scoped literals, `pnpm-workspace.yaml`) | TRANSITIVE CVE floor (GHSA-3jxr-9vmj-r5cp, high, exponential-time DoS — one advisory spanning all three major lines). Range-scoped so each coexisting major stays at its minimal patch per ADR-0018; validated 2026-07-20. The 3.x/5.x band was raised to **5.0.8** on 2026-07-28 for GHSA-mh99-v99m-4gvg; the 1.x/2.x lines have no backport and are allowlisted above (raising them breaks eslint) |
...
| `postcss`                          | `8.5.21`                     | catalog (`catalog:` override)                                | DIRECT (dual-role; the override references the catalog per ADR-0018). Raised 2026-07-28 for a high path-traversal in previous-source-map handling (CVE floor ≥ 8.5.18). Value is the latest stable **within the 7-day maturity buffer** — 8.5.22/8.5.23 were younger than the buffer on that date, so the direct-dep "latest stable" rule resolves to 8.5.21                                         |
...
| `hono`                             | `4.12.27`                    | `overrides` (literal, `pnpm-workspace.yaml`)                 | Was a de-dup pin at 4.12.26; promoted to TRANSITIVE CVE floor 2026-07-22 (3 moderate advisories — JSX context isolation, `cx()` XSS escape bypass, API-gateway adapter — all patched ≥4.12.27). Same chain as `@hono/node-server` (dev-only via `@prisma/dev`)                                                                                                                                       |
...
| `fast-uri`                         | `3.1.4`                      | `overrides` (range-scoped literal, `pnpm-workspace.yaml`)    | TRANSITIVE CVE floor added 2026-07-22 (2 moderate advisories — host/scheme confusion — patched ≥3.1.4). Tree resolved 3.1.2 (Fastify ajv chain); range-scoped `>=3.0.0 <3.1.4` per ADR-0018                                                                                                                                                                                                          |
