/**
 * @file SnapchatApiClient.writeFailFast.test.ts
 * @description RED tests: verify that SnapchatApiClient.createStory rejects on
 *              provider failure instead of resolving with a synthetic queued
 *              response. Drives the REAL SnapchatApiClient through the circuit
...
describe("SnapchatApiClient.createStory — write fail-fast (R2-F)", { concurrent: false }, () => {
  beforeAll(() => {
    mockFetchReject();
  });
  ...
  it("does NOT return a synthetic {id:'queued'} response on failure", async () => {
    ...
    try {
      resolved = await apiClient.createStory("test-media-id");
    } catch {
      rejected = true;
    }
    ...
    assert.ok(
      rejected,
      `createStory must reject on failure, but resolved with: ${JSON.stringify(resolved)}`
    );
  });
});
